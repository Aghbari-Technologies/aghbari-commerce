...
const requiredFiles = [
  'package.json', 'package-lock.json', 'vite.config.ts', 'playwright.config.ts', '.env.example', 'vercel.json',
  'index.html', 'src/main.tsx', 'src/App.tsx', 'src/lib/supabase.ts', 'supabase/config.toml',
];
const requiredWorkflows = [
  '.github/workflows/application-quality.yml', '.github/workflows/runtime-e2e.yml', '.github/workflows/security-audit.yml',
  '.github/workflows/supabase-migration-proof.yml', '.github/workflows/bootstrap-release-lockfile.yml',
];
const failures = [];
function fail(message) { failures.push(message); }

for (const file of requiredFiles) if (!existsSync(join(root, file))) fail(`Missing required release file: ${file}`);
for (const file of requiredWorkflows) if (!existsSync(join(root, file))) fail(`Missing required workflow: ${file}`);
...
if (existsSync(lockPath)) {
  try {
    ...
    for (const section of ['dependencies', 'devDependencies']) for (const [name, version] of Object.entries(pkg[section] ?? {})) {
      if (rootPackage?.[section]?.[name] !== version) fail(`Lockfile ${section} mismatch for ${name}: ${rootPackage?.[section]?.[name]} !== ${version}`);
    }
  } catch (error) { fail(`package-lock.json is not valid JSON: ${error.message}`); }
}
...
for (const [file, needles] of Object.entries(workflowChecks)) {
  ...
  for (const needle of needles) if (!text.includes(needle)) fail(`Release workflow contract missing '${needle}': ${file}`);
}

const sourceRoot = join(root, 'src');
const ignoredNames = new Set(['node_modules', 'dist', '.git']);
const suspiciousPatterns = [
  /\bTODO\b/i, /\bFIXME\b/i, /\bplaceholder\b/i, /\bnot\s+implemented\b/i, /\bnotimplemented\b/i,
...
function walk(dir) {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    const path = join(dir, entry), info = statSync(path);
    if (info.isDirectory()) result.push(...walk(path)); else result.push(path);
  }
  return result;
}
...
for (const file of sourceFiles) {
  ...
  // JSX placeholder props and the CSS ::placeholder selector are legitimate UI semantics,
  // not evidence of unfinished/mock implementation. Remove only those syntactic forms
  // before running the completion-marker detector so the audit remains strict elsewhere.
  const auditableText = text
    .replace(/\bplaceholder\s*=\s*["'][^"']*["']/gi, '')
    .replace(/::placeholder\b/gi, '');
  for (const pattern of suspiciousPatterns) if (pattern.test(auditableText) && !/(?:\.test\.|tests?\/)/i.test(rel)) {
    fail(`Suspicious completion/mock marker ${pattern} found in executable source: ${rel}`);
  }
  for (const { pattern, label } of forbiddenRuntimePatterns) if (pattern.test(text)) fail(`Forbidden runtime construct ${label} found in executable source: ${rel}`);
  if (/https?:\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?\b/i.test(text)) fail(`Hard-coded local runtime URL found in executable source: ${rel}`);
}
...
for (const file of ['index.html', 'public/manifest.webmanifest', 'vercel.json', '.env.example']) {
  ...
  for (const { pattern, label } of forbiddenRuntimePatterns) if (pattern.test(text)) fail(`Forbidden runtime construct ${label} found in release artifact: ${file}`);
}
...
} catch (error) { fail(`PWA manifest is not valid JSON: ${error.message}`); }

for (const script of ['test', 'lint', 'build', 'test:e2e', 'test:release-audit', 'typecheck']) if (!pkg.scripts?.[script]) fail(`Missing package script: ${script}`);
...
for (const key of ['VITE_SUPABASE_URL', 'VITE_SUPABASE_PUBLISHABLE_KEY']) if (!new RegExp(`^${key}=`, 'm').test(envExample)) fail(`Missing environment key in .env.example: ${key}`);
...
const migrationVersions = migrationFiles.map((file) => {
  const name = file.split(/[/\\]/).pop() ?? '', match = name.match(/^(\d+)_/);
  if (!match) fail(`Migration filename must begin with a numeric version: ${relative(root, file)}`);
  return match?.[1] ?? '';
}).filter(Boolean);
for (const version of [...new Set(migrationVersions.filter((version, index) => migrationVersions.indexOf(version) !== index))]) fail(`Duplicate migration version detected: ${version}`);
...
for (const name of [...rpcCalls].filter((name) => !new RegExp(`(?:create|replace)\\s+function\\s+(?:public\\.)?${name}\\s*\\(`, 'i').test(migrationText))) fail(`RPC contract missing from migration history: ${name}`);

const vercelText = readFileSync(join(root, 'vercel.json'), 'utf8');
for (const header of ['Content-Security-Policy', 'X-Content-Type-Options', 'X-Frame-Options', 'Strict-Transport-Security']) if (!vercelText.includes(`\"key\": \"${header}\"`)) fail(`Required production security header missing from vercel.json: ${header}`);
