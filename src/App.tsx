...
export default function App() {
  ...
  return <div className="customer-app" dir="rtl">
    ...
    <main className="customer-main">
      ...
      {section === 'orders' && <>{ordersError && <div className="error-banner" role="alert"><span>{ordersError}</span><button type="button" className="ghost" onClick={() => { setOrdersError(null); void getCustomerOrders(30).then(setOrders).catch((error) => setOrdersError(error instanceof Error ? error.message : 'تعذر تحميل الطلبات.')); }}>إعادة المحاولة</button></div>}<CustomerOrdersPanel orders={orders} loading={ordersLoading} detailBusy={orderDetailBusy} productsCount={products.length} onOpenDetail={(order) => void openOrderDetail(order)} onReorder={(order) => void reorder(order)} onReload={() => { setOrdersError(null); void getCustomerOrders(30).then(setOrders).catch((error) => setOrdersError(error instanceof Error ? error.message : 'تعذر تحميل الطلبات.')); }} /></>}
