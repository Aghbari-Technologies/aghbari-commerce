...
export default function App() {
  ...
  const [catalogLoading, setCatalogLoading] = useState(false); const [online, setOnline] = useState(() => typeof navigator === 'undefined' ? true : navigator.onLine);
  ...
  async function addToCart(product: Product) {
    const price = priceFor(product); if (price <= 0 || product.availableQuantity < 1) return;
    ...
    try { await setCartItem(product.id, quantity); setCart((current) => existing ? current.map((line) => line.product.id === product.id ? { ...line, quantity } : line) : [...current, { product, quantity, unitPrice: price }]); setCheckoutKey(null); setOrderResult(null); setRuntimeError(null); }
  ...
  async function updateQuantity(id: string, quantity: number) {
    ...
    try { if (next === 0) { await removeCartItem(id); setCart((current) => current.filter((item) => item.product.id !== id)); } else { await setCartItem(id, next); setCart((current) => current.map((item) => item.product.id === id ? { ...item, quantity: next } : item)); } setCheckoutKey(null); setRuntimeError(null); }
    catch (error) { setRuntimeError(error instanceof Error ? error.message : 'تعذر تحديث الكمية.'); }
  }

  async function submitOrder() {
    if (!customerId || !warehouseId || !online || !cart.length || orderBusy) return;
    ...
    try { const result = await createOrder({ customerId, idempotencyKey, lines: cart.map((line) => ({ productId: line.product.id, quantity: line.quantity })) }, warehouseId); setCart([]); setCheckoutKey(null); setOrderResult(result ? `تم إرسال الطلب رقم ${result.order_number} بنجاح.` : 'تم إرسال الطلب بنجاح.'); }
  ...
  return <div className="app-shell"><header className="topbar"><div className="brand"><span className="brand-mark">أ</span><div><strong>بوابة الأغبري</strong><small>للتجارة والجملة</small></div></div><nav aria-label="التنقل الرئيسي"><a className="active" href="#catalog">المنتجات</a>{STAFF_ROLES.has(role) && <a href="#account">مركز التحكم</a>}<a href="#orders">طلباتي</a></nav><button className="cart-button" aria-label={`السلة، ${cart.length} أصناف`} onClick={() => document.getElementById('cart')?.scrollIntoView({ behavior: 'smooth' })}>السلة <b>{cart.length}</b></button><button className="signout" onClick={() => void handleSignOut()}>خروج</button></header>
    <main><section className="hero" id="catalog"><div><span className="eyebrow">تجارة جملة أسرع</span><h1>اطلب احتياج متجرك<br/>بخطوات بسيطة.</h1><p>الكتالوج والسعر والمخزون تأتي من الخادم بعد التحقق من هوية العميل وتصنيفه.</p></div><div className="hero-card"><span>حالة الخدمة</span><strong>{!online ? 'غير متصل' : runtimeError ? 'يحتاج انتباهًا' : 'متصل'}</strong><small>{!online ? 'الاتصال بالإنترنت مطلوب لتحديث الأسعار والمخزون وإرسال الطلبات.' : runtimeError ?? 'الأسعار المعروضة هي السعر الفعّال المصرح به لحسابك.'}</small></div></section>
      {runtimeError && <div className="error-banner" role="alert">{runtimeError}</div>}
      <section className="toolbar"><label className="search"><span className="search-icon" aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث بالاسم أو SKU..." aria-label="بحث المنتجات" /></label><div className="chips" aria-label="تصنيف المنتجات">{categories.map((item) => <button key={item.id ?? 'all'} className={item.id === categoryId ? 'chip selected' : 'chip'} onClick={() => setCategoryId(item.id)}>{item.name}</button>)}</div></section>
