...
export default function App() {
  ...
  const [finance, setFinance] = useState<Finance | null>(null); const [financeLoading, setFinanceLoading] = useState(false); const [financeError, setFinanceError] = useState<string | null>(null); const [financeReloadKey, setFinanceReloadKey] = useState(0); const [templates, setTemplates] = useState<OrderTemplate[]>([]); const [templateName, setTemplateName] = useState('');
  ...
  return <div className="customer-app" dir="rtl">
    ...
    <main className="customer-main">
      ...
      {section === 'orders' && <>{ordersError && <div className="error-banner" role="alert"><span>{ordersError}</span><button type="button" className="ghost" onClick={() => { setOrdersLoading(true); setOrdersError(null); void getCustomerOrders(30).then(setOrders).catch((error) => setOrdersError(error instanceof Error ? error.message : 'تعذر تحميل الطلبات.')).finally(() => setOrdersLoading(false)); }}>إعادة المحاولة</button></div>}<CustomerOrdersPanel orders={orders} loading={ordersLoading} detailBusy={orderDetailBusy} productsCount={products.length} onOpenDetail={(order) => void openOrderDetail(order)} onReorder={(order) => void reorder(order)} onReload={() => { setOrdersLoading(true); setOrdersError(null); void getCustomerOrders(30).then(setOrders).catch((error) => setOrdersError(error instanceof Error ? error.message : 'تعذر تحميل الطلبات.')).finally(() => setOrdersLoading(false)); }} /></>}
...
function QuickOrderEditor({ onResolveIdentifier, onClose }: { onResolveIdentifier: (identifier: string, quantity: number) => Promise<void>; onClose: () => void }) {
