    ...
    async function loadRuntime() {
      ...
      try {
        const [{ data: warehouse, error: warehouseError }, items, savedCart] = await Promise.all([
          supabase.from('warehouses').select('id').eq('is_active', true).order('created_at').limit(1).maybeSingle(), getCatalog(query, null, 100, 0), getCart()
        ]);
        if (warehouseError) throw warehouseError; if (!warehouse?.id) throw new Error('لا يوجد مستودع تشغيلي نشط.'); if (cancelled) return;
        const imageUrls = await getProductImageUrls(items.map((item) => item.image_path)); if (cancelled) return;
