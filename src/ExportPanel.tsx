...
async function fetchAllProducts() {
  ...
  for (let from = 0; from < MAX_EXPORT_ROWS; from += PAGE_SIZE) {
    const { data, error } = await supabase!
      .from('products').select('id,sku,name,unit,status,created_at').order('name')
...
export default function ExportPanel({ role }: { role: UserRole }) {
  ...
  async function exportProducts() {
    ...
    try {
      ...
      const [products, priceResult] = await Promise.all([
        fetchAllProducts(),
        supabase.from('product_prices').select('product_id,amount,valid_from,valid_to,price_lists!inner(tier,currency)').lte('valid_from', now).or(`valid_to.is.null,valid_to.gte.${now}`).order('valid_from', { ascending: false }).limit(MAX_PRICE_ROWS)
      ]);
      ...
      for (const row of priceResult.data ?? []) {
        ...
        const key = tier as 'retail' | 'wholesale' | 'distributor';
        if (current[key] === undefined) current[key] = Number(row.amount);
  ...
  return <div className="admin-card" id="exports"><h3>تصدير بيانات التشغيل</h3><p>تصدير الكتالوج والأسعار المصرح بها كملف CSV متوافق مع Excel.</p><button disabled={busy} onClick={() => void exportProducts()}>{busy ? 'جارٍ التصدير…' : 'تصدير الكتالوج والأسعار'}</button>{error && <div className="error-banner" role="alert">{error}</div>}{message && <div className="success" role="status">{message}</div>}</div>;
}
