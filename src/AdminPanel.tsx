...
import { getStaffOrderDetail, getStaffOrders, transitionOrder, type StaffOrderDetail, type StaffOrderSummary } from './services/staffOrders';
...
export default function AdminPanel({ role }: { role: UserRole }) {
  ...
  const [product, setProduct] = useState({ sku: '', name: '', unit: 'كرتون', categoryId: '', description: '' }); const [category, setCategory] = useState({ name: '', slug: '', parentId: '' }); const [selectedProduct, setSelectedProduct] = useState(''); const [tier, setTier] = useState<CustomerTier>('wholesale'); const [price, setPrice] = useState(''); const [warehouseId, setWarehouseId] = useState(''); const [delta, setDelta] = useState(''); const [reason, setReason] = useState(''); const [imageFile, setImageFile] = useState<File | null>(null); const [importFile, setImportFile] = useState<File | null>(null); const [importJobId, setImportJobId] = useState<string | null>(null); const [importPreview, setImportPreview] = useState<{ rows: number; invalid: number } | null>(null); const [busy, setBusy] = useState(false); const [message, setMessage] = useState<string | null>(null); const [error, setError] = useState<string | null>(null); const [detailOrder, setDetailOrder] = useState<StaffOrderDetail | null>(null); const [detailOrderBusy, setDetailOrderBusy] = useState(false);
  ...
  async function openOrderDetail(orderId: string) {
    setDetailOrderBusy(true); setError(null);
    try { setDetailOrder(await getStaffOrderDetail(orderId)); }
    catch (e) { setError(e instanceof Error ? e.message : 'تعذر تحميل تفاصيل الطلب التشغيلي.'); }
    finally { setDetailOrderBusy(false); }
  }
  ...
  return <section className="admin-panel" id="account">
    ...
    <details className="admin-operations" open>
      ...
      <div className="admin-grid">
        ...
        {canCatalog && <form className="admin-card" id="admin-import" onSubmit={(e) => { e.preventDefault(); void stageImport(); }}><h3>استيراد Excel آمن</h3><input aria-label="ملف المنتجات" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(e) => { setImportFile(e.target.files?.[0] ?? null); setImportJobId(null); setImportPreview(null); }} required />{importPreview && <small>الصفوف: {importPreview.rows} · الأخطاء: {importPreview.invalid}</small>}{!importJobId ? <button disabled={busy || !importFile}>رفع ومعاينة</button> : <button disabled={busy || !warehouseId} onClick={(e) => { e.preventDefault(); void commitImport(); }}>اعتماد الاستيراد الذري</button>}</form>}
