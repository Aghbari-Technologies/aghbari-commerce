...
export default function AdminPanel({ role }: { role: UserRole }) {
  ...
  useEffect(() => { const onKeyDown = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); setCommandOpen(true); setCommandQuery(''); } if (event.key === 'Escape') setCommandOpen(false); }; window.addEventListener('keydown', onKeyDown); return () => window.removeEventListener('keydown', onKeyDown); }, []);
  ...
  const canCatalog = role === 'owner' || role === 'admin' || role === 'sales'; const canCategory = role === 'owner' || role === 'admin'; const canInventory = role === 'owner' || role === 'admin' || role === 'warehouse'; const canOrderWorkflow = STAFF_ROLES.has(role); const canFinance = ['owner', 'admin', 'sales'].includes(role); const visibleOrders = orders.filter((order) => { const needle = orderQuery.trim().toLowerCase(); if (!needle) return true; return String(order.order_number).includes(needle) || String(order.customer_name ?? '').toLowerCase().includes(needle) || String(STATUS_LABELS[order.status] ?? order.status).toLowerCase().includes(needle); }); const commands = [
    ['فتح الطلبات', '#admin-orders', canOrderWorkflow], ['إدارة العملاء', '#admin-customers', canCatalog], ['إضافة منتج', '#admin-product-create', canCatalog],
    ['التصنيفات', '#admin-category-create', canCategory], ['التسعير', '#admin-pricing', canCatalog], ['صور المنتجات', '#admin-product-image', canCatalog],
    ['الاستيراد الآمن', '#admin-import', canCatalog], ['المخزون', '#admin-inventory', canInventory], ['تعديل المخزون', '#admin-inventory-adjust', canInventory],
  ...
  return <section className="admin-panel" id="account">
    <AdminExecutiveDashboard role={role} />
      ...
      <button type="button" className="admin-command-trigger" aria-haspopup="dialog" aria-expanded={commandOpen} onClick={() => { setCommandOpen(true); setCommandQuery(''); }}>⌘ فتح مركز الأوامر <kbd>Ctrl K</kbd></button>
      {commandOpen && <div className="admin-command-backdrop" role="presentation" onClick={() => setCommandOpen(false)}>
        <section className="admin-command-dialog" role="dialog" aria-modal="true" aria-labelledby="admin-command-title" onClick={(event) => event.stopPropagation()}>
          ...
          <div className="admin-command-results">{visibleCommands.length ? visibleCommands.map(([label,target,allowed]) => allowed && <a key={target} href={target} onClick={() => setCommandOpen(false)}>{label}<span>↗</span></a>) : <div className="cart-empty">لا توجد إجراءات مطابقة.</div>}</div>
          <small>اختصار لوحة المفاتيح: Ctrl+K أو ⌘K · الإجراءات الظاهرة مقيدة بدورك الحالي.</small>
        </section>
      </div>}
