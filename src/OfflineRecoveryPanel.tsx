...
export default function OfflineRecoveryPanel({ userId }: { userId: string | null }) {
 const [rows,setRows]=useState<OfflineOperation[]>([]),[filter,setFilter]=useState<RecoveryFilter>('problems');
 const reload=useCallback(()=>setRows(userId?pendingOfflineOperations(userId):[]),[userId]);
 useEffect(()=>{reload();const timer=window.setInterval(reload,4000);return()=>window.clearInterval(timer);},[reload]);
 ...
 return <section className="offline-recovery-panel" aria-labelledby="offline-recovery-title">
  <div className="section-title"><div><span className="eyebrow">Recovery Center</span><h2 id="offline-recovery-title">مركز التعارض والاسترداد</h2><p>حالات السلة المحلية فقط؛ المصدر الخادمي يظل صاحب القرار النهائي.</p></div><span>{problems.length?problems.length+' تحتاج إجراء':rows.length+' عمليات مؤجلة'}</span></div>
  ...
  {!visible.length?<div className="empty-state"><strong>لا توجد مشكلات معلقة.</strong><span>ستظهر التعارضات أو حالات الفشل هنا عند الحاجة.</span></div>:<div className="offline-recovery-list">{visible.map(operation=><article key={operation.operationId}><div><strong>{label(operation)}</strong><small>{operation.type==='cart:set_item'?'تحديث كمية السلة':'إزالة صنف من السلة'} · {new Date(operation.createdAt).toLocaleString('ar-YE')}</small></div><p>{explanation(operation)}</p>{(operation.state==='conflicted'||operation.state==='terminal')&&<button type="button" className="ghost" onClick={()=>{removeOfflineOperation(operation.operationId);reload();}}>إزالة من الطابور</button>}</article>)}</div>}
 </section>;
}
