...
export async function getStaffOrders(limit = 50): Promise<StaffOrderSummary[]> {
  ...
  const { data, error } = await requireSupabase()
    .from('orders')
    .select('id,order_number,customer_id,warehouse_id,status,total,currency,created_at,updated_at')
...
export async function transitionOrder(orderId: string, toStatus: OrderStatus): Promise<StaffOrderSummary> {
  ...
  const { data, error } = await requireSupabase().rpc('transition_order', {
    p_order_id: orderId,
