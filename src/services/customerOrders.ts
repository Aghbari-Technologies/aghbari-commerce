...
export async function getCustomerOrders(limit = 20): Promise<CustomerOrderSummary[]> {
  ...
  return (data ?? []).map((item) => ({ ...(item as Omit<CustomerOrderSummary, 'order_number' | 'total'>), order_number: Number(item.order_number), total: finiteNumber(item.total) })) as CustomerOrderSummary[];
}
