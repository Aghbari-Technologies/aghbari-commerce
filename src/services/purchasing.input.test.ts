...
describe('purchasing input boundaries', () => {
  ...
  it('rejects malformed supplier or warehouse ids', () => {
    expect(() => validatePurchaseOrderInput({ ...validPurchase, supplierId: 'bad' })).toThrow();
    expect(() => validatePurchaseOrderInput({ ...validPurchase, warehouseId: 'bad' })).toThrow();
  });
  it('rejects missing lines and weak idempotency', () => {
