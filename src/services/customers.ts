...
export function validateCustomerInput(name: string, phone: string, tier: CustomerTier): { name: string; phone: string | null; tier: CustomerTier } {
  const normalizedName = name.trim();
  if (!normalizedName || normalizedName.length > 200) throw new Error('اسم العميل مطلوب وبحد أقصى 200 حرف.');
  const normalizedPhone = phone.trim();
  if (normalizedPhone.length > 50) throw new Error('رقم هاتف العميل طويل جدًا.');
...
export function validateCustomerTier(tier: CustomerTier): CustomerTier {
  if (!CUSTOMER_TIERS.has(tier)) throw new Error('فئة العميل غير مسموحة.');
  return tier;
}
...
export async function createCustomer(name: string, phone: string, tier: CustomerTier) {
  const input = validateCustomerInput(name, phone, tier);
...
export async function setCustomerTier(customerId: string, tier: CustomerTier) {
  ...
  const normalizedTier = validateCustomerTier(tier);
...
export async function setCustomerActive(customerId: string, isActive: boolean) {
  const id = requireUuid(customerId, 'العميل');
  if (typeof isActive !== 'boolean') throw new Error('حالة العميل غير صالحة.');
