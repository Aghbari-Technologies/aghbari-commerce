...
async function replayCartOperation(operation: OfflineOperation): Promise<void> {
  ...
  if (!session?.user?.id || session.user.id !== operation.userId) {
    throw new Error('لا يمكن إعادة تشغيل عملية سلة تخص حسابًا آخر.');
  }
...
export async function syncOfflineCart() {
  const session = await getSession();
  if (!session?.user?.id) return { processed: 0, failed: 0 };
  return drainOfflineOperations(replayCartOperation, session.user.id);
}
...
export async function setCartItem(productId: string, quantity: number) {
  ...
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    const session = await getSession();
    if (!session?.user?.id) throw new Error('تسجيل الدخول مطلوب لحفظ عملية السلة دون اتصال.');
    enqueueOfflineOperation(session.user.id, OFFLINE_CART_SET_ITEM, { productId: productId.trim(), quantity });
    return;
  }
...
export async function removeCartItem(productId: string) {
  ...
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    const session = await getSession();
    if (!session?.user?.id) throw new Error('تسجيل الدخول مطلوب لحفظ عملية السلة دون اتصال.');
    enqueueOfflineOperation(session.user.id, OFFLINE_CART_REMOVE_ITEM, { productId: productId.trim() });
    return;
  }
