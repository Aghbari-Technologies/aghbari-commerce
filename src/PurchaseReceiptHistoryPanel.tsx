...
export default function PurchaseReceiptHistoryPanel({role}:{role:UserRole}){
  ...
  const [query,setQuery]=useState(''); const [page,setPage]=useState(1); const [loading,setLoading]=useState(true); const [error,setError]=useState<string|null>(null); const [selectedReceipt,setSelectedReceipt]=useState<Receipt|null>(null);
...
<div className='ops-metrics-strip' aria-label='ملخص الاستلام'><article><small>الإيصالات</small><strong>{receipts.length.toLocaleString('ar')}</strong><span>إجمالي السجل</span></article><article><small>النتائج</small><strong>{filtered.length.toLocaleString('ar')}</strong><span>بعد البحث</span></article><article><small>الأوامر</small><strong>{orders.size.toLocaleString('ar')}</strong><span>أوامر مرتبطة</span></article><article><small>الموردون</small><strong>{suppliers.size.toLocaleString('ar')}</strong><span>موردون مرتبطون</span></article></div>
