...
import { getPurchaseReceiptDetail } from './services/purchasing';
...
export default function PurchaseReceiptHistoryPanel({role}:{role:UserRole}){
  ...
  const [query,setQuery]=useState(''); const [page,setPage]=useState(1); const [loading,setLoading]=useState(true); const [error,setError]=useState<string|null>(null); const [selectedReceipt,setSelectedReceipt]=useState<Receipt|null>(null); const [receiptDetail,setReceiptDetail]=useState<Awaited<ReturnType<typeof getPurchaseReceiptDetail>>|null>(null); const [detailBusy,setDetailBusy]=useState(false);
  ...
  async function openReceiptDetail(id:string){setDetailBusy(true);setError(null);try{setReceiptDetail(await getPurchaseReceiptDetail(id));}catch(e){setError(e instanceof Error?e.message:'تعذر تحميل تفاصيل إيصال الاستلام.');}finally{setDetailBusy(false);}}useEffect(()=>{setPage(1);},[query]);
