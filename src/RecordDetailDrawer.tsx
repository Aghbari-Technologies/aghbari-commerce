            ...
            return (
              <div className={field.wide ? 'record-detail-field wide' : 'record-detail-field'} key={field.label}>
                <div className="record-detail-field-label">
                  ...
                  {text !== null && <button type="button" className="record-detail-copy" onClick={() => void copyField(field.label, field.value)} aria-label={`نسخ ${field.label}`} aria-live="polite">{copiedLabel === field.label ? 'تم النسخ' : 'نسخ'}</button>}
                </div>
