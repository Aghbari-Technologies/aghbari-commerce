...
export default function FinancePanel({ role }: { role: UserRole }) {
  ...
  const [busy,setBusy]=useState(false); const [loading,setLoading]=useState(true); const [message,setMessage]=useState<string|null>(null); const [error,setError]=useState<string|null>(null);

  const reload=useCallback(async()=>{
    if(!supabase || (!canInvoice && !canExpense && !canAccount)){setLoading(false);return;}
    setLoading(true);
  ...
  async function run(action:()=>Promise<unknown>,success:string){setBusy(true);setError(null);setMessage(null);try{await action();setMessage(success);await reload();}catch(e){setLoading(false);setError(e instanceof Error?e.message:'تعذر تنفيذ العملية.');}finally{setBusy(false);}}
  if(!canInvoice&&!canExpense&&!canAccount)return null;

  return <div className="cart-panel" id="finance">
    <div className="section-heading"><div><span className="eyebrow">المالية التشغيلية</span><h2>الفواتير والتحصيل والمصروفات</h2></div><span aria-live="polite">{loading ? 'جارٍ التحديث…' : `${cash.length} حسابات نقدية`}</span></div>
    ...
    {error&&<div className="error-banner" role="alert"><span>{error}</span><button type="button" className="ghost" onClick={()=>void reload()} disabled={loading}>إعادة تحميل المالية</button></div>}{message&&<div className="success" role="status">{message}</div>}
  </div>;
}
