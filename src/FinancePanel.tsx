...
import { createInvoiceFromOrder, getCashBalances, getInvoices, recordExpense, recordPayment, type CashBalance, type OperationalInvoice } from './services/finance';
...
export default function FinancePanel({ role }: { role: UserRole }) {
  ...
  const reload=useCallback(async()=>{
    if(!supabase || (!canInvoice && !canExpense)) return;
    ...
    if(!expenseAccountId && cashRows[0]) setExpenseAccountId(cashRows[0].id);
    if(!branchId && branchRows?.[0]) setBranchId(branchRows[0].id);
  },[branchId,canExpense,canInvoice,cashAccountId,expenseAccountId,invoiceId]);
  ...
  return <div className="cart-panel" id="finance">
    ...
    <div className="admin-grid">
      ...
      {canInvoice&&<form className="admin-card" onSubmit={e=>{e.preventDefault();if(!invoiceId||!amount)return;void run(()=>recordPayment(invoiceId,Number(amount),method,method==='cash'?cashAccountId:null,reference),'تم تسجيل التحصيل وتحديث حالة الفاتورة.')}}>
        <h3>تسجيل تحصيل</h3><select aria-label="الفاتورة" value={invoiceId} onChange={e=>setInvoiceId(e.target.value)} required><option value="">اختر فاتورة</option>{invoices.filter(i=>i.status!=='paid'&&i.status!=='void').map(i=><option key={i.id} value={i.id}>فاتورة #{i.invoice_number} · {i.total} {i.currency}</option>)}</select>
        <input aria-label="مبلغ التحصيل" type="number" min="0.01" step="0.01" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="مبلغ التحصيل" required />
        <select aria-label="طريقة الدفع" value={method} onChange={e=>setMethod(e.target.value as PaymentMethod)}>{(Object.keys(methodLabels) as PaymentMethod[]).map(m=><option key={m} value={m}>{methodLabels[m]}</option>)}</select>
        {method==='cash'&&<select aria-label="الحساب النقدي" value={cashAccountId} onChange={e=>setCashAccountId(e.target.value)} required><option value="">حساب النقدية</option>{cash.map(a=><option key={a.id} value={a.id}>{a.name} · {a.currency}</option>)}</select>}
        <input aria-label="مرجع التحصيل" value={reference} onChange={e=>setReference(e.target.value)} placeholder="مرجع (اختياري)" /><button disabled={busy||!invoiceId}>تسجيل التحصيل</button>
      </form>}
      {canExpense&&<form className="admin-card" onSubmit={e=>{e.preventDefault();if(!branchId||!expenseAccountId||!expenseAmount)return;void run(()=>recordExpense(branchId,expenseAccountId,expenseCategory,Number(expenseAmount),cash.find(a=>a.id===expenseAccountId)?.currency??'YER',expenseDescription),'تم تسجيل المصروف وتحديث دفتر النقدية.')}}>
        <h3>مصروف تشغيلي</h3><select aria-label="الفرع" value={branchId} onChange={e=>setBranchId(e.target.value)} required><option value="">اختر الفرع</option>{branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select>
        <select aria-label="حساب المصروف" value={expenseAccountId} onChange={e=>setExpenseAccountId(e.target.value)} required><option value="">حساب النقدية</option>{cash.map(a=><option key={a.id} value={a.id}>{a.name} · {a.currency}</option>)}</select>
        <input aria-label="فئة المصروف" value={expenseCategory} onChange={e=>setExpenseCategory(e.target.value)} placeholder="الفئة" required /><input aria-label="مبلغ المصروف" type="number" min="0.01" step="0.01" value={expenseAmount} onChange={e=>setExpenseAmount(e.target.value)} placeholder="المبلغ" required /><input aria-label="وصف المصروف" value={expenseDescription} onChange={e=>setExpenseDescription(e.target.value)} placeholder="الوصف" /><button disabled={busy}>تسجيل المصروف</button>
      </form>}
