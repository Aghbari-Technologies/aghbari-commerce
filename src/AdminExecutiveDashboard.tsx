    ...
    <div className="executive-layout">
      ...
      <div className="executive-content" id="admin-dashboard">
        ...
        <div className="executive-grid-two">
          ...
          <article className="executive-card status-distribution-card"><div className="executive-card-title"><div><span>توزيع التشغيل</span><h2>حالة الطلبات</h2></div><b>{metricErrors.orders ? '—' : orders.length}</b></div>{metricErrors.orders ? <div className="executive-degraded">تعذر تحديث حالات الطلبات من المصدر التشغيلي.</div> : statusCounts.length ? <div className="status-donut-layout"><div className="status-donut" style={{ background: (() => { const total = Math.max(orders.length, 1); let cursor = 0; return `conic-gradient(${statusCounts.map((item) => { const start = (cursor / total) * 100; cursor += item.count; const end = (cursor / total) * 100; return `var(--status-${item.status}, #1599ad) ${start}% ${end}%`; }).join(', ')})`; })() }} aria-label="توزيع حالات الطلبات"><span>{orders.length}</span><small>طلب</small></div><div className="status-donut-legend">{statusCounts.map((item) => <div key={item.status}><span className="status-dot" style={{ background: `var(--status-${item.status}, #1599ad)` }} /><strong>{item.label}</strong><b>{item.count}</b></div>)}</div></div> : <p className="executive-empty">لا توجد طلبات تشغيلية في العينة الحالية.</p>}</article>
        </div>
