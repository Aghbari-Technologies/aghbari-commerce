...
test('real admin-to-customer invitation journey', async ({ page, context }) => {
  ...
  page.on('response', async (response) => {
    if (!response.url().includes('/functions/v1/customer-invitations')) return;
    let body = '';
    try { body = await response.text(); } catch {}
    invitationResponses.push(`${response.status()} ${body}`);
  });
  ...
  try {
    ...
    throw new Error(`Invitation link missing. Function responses: ${invitationResponses.join(' || ')}. Original: ${error instanceof Error ? error.message : String(error)}`);
  }
