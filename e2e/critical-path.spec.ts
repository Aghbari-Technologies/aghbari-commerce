...
test('quick order resolves a server-authorized product outside the currently loaded catalog results', async ({ page }) => { const email = process.env.E2E_EMAIL; const password = process.env.E2E_PASSWORD; if (!email || !password) throw new Error('E2E_EMAIL and E2E_PASSWORD are required for server-backed quick-order lookup proof.'); const failures = captureBrowserFailures(page); await login(page, email, password); const search = page.getByRole('textbox', { name: 'البحث في الكتالوج' }); await search.fill('BROW-001'); await expect(page.locator('.product-card')).toHaveCount(1); const quickButton = page.getByRole('button', { name: 'طلب سريع', exact: true }); await quickButton.click(); const identifierInput = page.getByRole('textbox', { name: 'SKU / الباركود' }); const qtyInput = page.getByRole('spinbutton', { name: 'كمية الطلب' }); await expect(identifierInput).toBeFocused(); await identifierInput.fill('BROW-002'); await qtyInput.fill('1'); await identifierInput.press('Enter'); await expect(page.getByRole('button', { name: /السلة/ })).toContainText('1'); await assertCleanBrowser(failures); });
