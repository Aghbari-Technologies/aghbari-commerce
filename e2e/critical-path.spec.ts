...
test('inline product quantity controls preserve a single cart line', async ({ page }) => {
  ...
  const card = page.locator('.product-card').first(); await expect(card).toBeVisible(); const addButton = card.getByRole('button', { name: 'إضافة للسلة' }); if (await addButton.count()) {
    await addButton.click(); await expect(card.locator('.product-qty-control')).toBeVisible(); await expect(card.locator('.product-qty-control output')).toHaveText('1'); await card.getByRole('button', { name: /زيادة/ }).click(); await expect(card.locator('.product-qty-control output')).toHaveText('2'); await card.getByRole('button', { name: /إنقاص/ }).click(); await expect(card.locator('.product-qty-control output')).toHaveText('1');
  }
});
