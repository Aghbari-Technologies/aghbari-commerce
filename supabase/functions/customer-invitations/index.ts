...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === 'accept') {
      ...
      if (!/^[0-9a-f]{64}$/.test(token) || password.length < 8 || password.length > 256) return json({ error: 'invalid_acceptance_payload' }, 422);
      ...
      if (!invitation) return json({ error: 'invitation_invalid_or_expired' }, 403);
