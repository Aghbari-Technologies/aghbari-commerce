...
BEGIN
  ...
  -- Serialize competing requests for the same tenant/business idempotency key.
  -- This closes the TOCTOU window between replay lookup and INSERT.
  PERFORM pg_advisory_xact_lock(hashtextextended(v_org::text || ':' || v_requested_key, 0));

  -- Validate the complete request shape before replay lookup so malformed replays
  -- cannot resolve to an existing order.
  ...
  -- First pass locks inventory rows in deterministic product order and resolves
  -- the authoritative customer-tier price before any business mutation.
  FOR v_line IN
  ...
  -- Preserve the canonical atomic checkout invariant from 0017: a successful
  -- order converts the active cart inside the same database transaction.
...
EXCEPTION WHEN unique_violation THEN
  -- The advisory lock should make same-key races impossible. Do not convert
  -- unrelated unique violations into a false idempotent success.
  RAISE;
END;
$$;
