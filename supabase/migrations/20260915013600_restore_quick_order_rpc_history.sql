-- Restore the quick-order RPC to migration history.
-- The function already exists in production, but release-audit requires every
-- frontend RPC contract to be reproducible from the source migration set.
...
begin
  ...
  insert into public.audit_events(
    ...
    v_org,auth.uid(),'cart.quick_order.import','cart',v_cart,'success',
    jsonb_build_object(
      'line_count',v_count,
      'subtotal',v_subtotal,
...
revoke execute on function public.apply_quick_order(text, uuid, jsonb) from anon, public;
