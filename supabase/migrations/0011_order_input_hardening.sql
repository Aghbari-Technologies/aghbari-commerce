-- Harden the customer order command at the database boundary.
-- These checks complement the TypeScript domain limits; the database remains authoritative.
...
begin
  ...
  if exists (
    ...
    having count(*) > 1
  ) then raise exception using errcode='22023', message='duplicate product line'; end if;
...
comment on function public.create_order(text,uuid,jsonb) is 'Atomic customer order command with server-authoritative pricing, stock locking, idempotency, duplicate-line rejection, and bounded order input.';
