-- Secure RPC boundary for order creation.
-- The client-provided actor parameter remains in the compatibility signature but is ignored;
-- the authenticated Supabase user is always derived from auth.uid().
...
as $$
declare
  v_actor_user_id uuid := (select private.current_app_user_id());
  v_org_id uuid;
...
begin
  ...
  if p_operation_id is null or p_customer_id is null or p_branch_id is null or p_warehouse_id is null then
    raise exception 'required identifiers are missing' using errcode = '22023';
  end if;
  if jsonb_typeof(p_lines) <> 'array' or jsonb_array_length(p_lines) = 0 then
    raise exception 'at least one order line is required' using errcode = '22023';
  end if;

  select c.organization_id, c.user_id, c.status, c.tier_id
    into v_org_id, v_customer_user_id, v_customer_status, v_tier_id
  ...
  -- A customer may create only their own order. Staff require explicit sales scope.
  if v_customer_user_id = v_actor_user_id then
    null;
  elsif not (
    (select private.current_app_role('system_admin'))
    or (select private.current_app_role('sales_manager'))
    or (select private.current_app_role('sales_employee'))
    or (select private.current_app_role('moderator'))
  ) then
    raise exception 'forbidden order actor' using errcode = '42501';
  end if;
  ...
  if not exists (
    ...
    raise exception 'branch outside authorized organization' using errcode = '42501';
  end if;
  if not exists (
    select 1 from public.warehouses w
    where w.id = p_warehouse_id and w.organization_id = v_org_id and w.branch_id = p_branch_id and w.active
  ) then
    raise exception 'warehouse outside authorized branch' using errcode = '42501';
  end if;

  if not (select private.can_access_branch(p_branch_id)) and v_customer_user_id <> v_actor_user_id then
    raise exception 'forbidden branch scope' using errcode = '42501';
  end if;
  if not (select private.can_access_warehouse(p_warehouse_id)) and v_customer_user_id <> v_actor_user_id then
    raise exception 'forbidden warehouse scope' using errcode = '42501';
  end if;
  ...
  where organization_id = v_org_id and operation_id = p_operation_id;
  if found then
  ...
  -- Lock all required inventory rows in deterministic product order before mutation.
  for v_product in
  ...
  loop
    if not exists (
      ...
      where p.id = v_product.product_id and p.organization_id = v_org_id and p.status = 'ACTIVE'
    ) then
    ...
    where ib.organization_id = v_org_id and ib.warehouse_id = p_warehouse_id and ib.product_id = v_product.product_id
    for update;
    ...
    join public.price_lists pl on pl.id = pp.price_list_id and pl.organization_id = pp.organization_id
    where pp.organization_id = v_org_id and pp.product_id = v_product.product_id
      and pl.tier_id = v_tier_id and pp.active and pl.active
      and pp.effective_from <= now() and (pp.effective_to is null or now() < pp.effective_to)
  ...
  loop
    ...
    where organization_id = v_org_id and warehouse_id = p_warehouse_id and product_id = v_line.product_id;
    ...
    insert into public.inventory_movements(
      ...
      'ORDER', v_order_id::text, p_operation_id, v_actor_user_id, 'Order acceptance', now());
  end loop;

  insert into public.order_status_history(
    organization_id, order_id, from_status, to_status, actor_user_id, correlation_id
  ) values (v_org_id, v_order_id, null, 'NEW', v_actor_user_id, p_correlation_id);
  ...
  insert into public.audit_events(
    ...
    v_org_id, v_actor_user_id, 'order.create', 'order', v_order_id,
...
revoke execute on function public.create_order_transaction(uuid, uuid, uuid, uuid, jsonb, numeric, uuid, uuid) from public, anon;
grant execute on function public.create_order_transaction(uuid, uuid, uuid, uuid, jsonb, numeric, uuid, uuid) to authenticated;
