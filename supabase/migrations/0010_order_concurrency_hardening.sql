-- Concurrency and abuse hardening for the server-authoritative order command.
-- Keeps the existing contract intact while making lock acquisition deterministic and bounded.
...
begin
  ...
  -- Reject duplicate product lines before any stock mutation. Lock/order traversal is
  -- deterministic by product UUID to prevent avoidable deadlocks between concurrent orders.
  if exists (
    select 1
    from (
...
comment on function public.create_order(text,uuid,jsonb) is 'Atomic customer order command with server-authoritative pricing, deterministic stock locking, duplicate-line rejection and idempotent replay.';
