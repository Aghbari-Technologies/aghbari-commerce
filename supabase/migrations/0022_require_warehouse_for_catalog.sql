-- Replace the warehouse-aware catalog function with a no-default signature.
-- This makes warehouse selection mandatory and removes legacy call-shape ambiguity.
...
returns table (
  id uuid, sku text, name text, unit text, category_id uuid, description text,
  status text, available_quantity integer, image_path text, authorized_price numeric, currency text
)
...
begin
  ...
  return query
  select p.id,p.sku,p.name,p.unit,p.category_id,p.description,p.status,
    coalesce((select ib.quantity from public.inventory_balances ib where ib.organization_id=v_org and ib.product_id=p.id and ib.warehouse_id=p_warehouse_id order by ib.updated_at desc limit 1),0),
...
$$;
revoke execute on function public.get_catalog(text,uuid,integer,integer,uuid) from public;
revoke execute on function public.get_catalog(text,uuid,integer,integer,uuid) from anon;
