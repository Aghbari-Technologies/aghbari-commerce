...
as $$
declare
  v_org uuid:=public.current_organization_id(); v_customer uuid:=public.current_customer_id(); v_order public.orders%rowtype; v_existing public.orders%rowtype; v_line jsonb; v_product uuid; v_qty integer; v_price numeric(18,2); v_tier public.customer_tier; v_currency text; v_subtotal numeric(18,2):=0; v_available integer; v_line_count integer; v_existing_count integer; v_requested_key text:=pg_catalog.btrim(coalesce(p_idempotency_key,''));
begin
 ...
 select c.tier into v_tier from public.customers c where c.id=v_customer and c.organization_id=v_org and c.is_active; if v_tier is null then raise exception using errcode='42501',message='active customer required'; end if;
 ...
 for v_line in select value from pg_catalog.jsonb_array_elements(p_lines) loop
  if pg_catalog.btrim(v_line->>'product_id') is null or pg_catalog.btrim(v_line->>'product_id')='' then raise exception using errcode='22023',message='product_id required'; end if;
