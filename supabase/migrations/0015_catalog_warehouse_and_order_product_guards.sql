-- Catalog stock must come from the same active warehouse used by the storefront.
-- Order creation must reject inactive/nonexistent products before pricing and mutation.
...
begin
  ...
  if v_warehouse is null then
    raise exception using errcode = 'P0002', message = 'no active warehouse available';
  end if;
  ...
  select
    ...
    coalesce((select ib.quantity from public.inventory_balances ib
              where ib.organization_id = v_org and ib.warehouse_id = v_warehouse and ib.product_id = p.id), 0) as available_quantity,
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    if not found or v_product_status <> 'active' then
      raise exception using errcode='P0002', message='product not available';
    end if;
