-- Expense posting must never drive an operational cash account below zero.
-- The account row is locked before the balance is evaluated so concurrent expense posts serialize.
...
BEGIN
  ...
  GROUP BY v_account.opening_balance;
