...
begin
  ...
  select greatest(1, least(coalesce((config->>'maxTemplates')::integer, 50), 500)) into v_max_templates
  from public.client_ui_settings where organization_id=v_org;
  if (select count(*) from public.order_templates where organization_id=v_org and customer_id=v_customer) >= v_max_templates then
