-- product_prices_staff_read is authoritative from migration 0028_staff_product_price_read.sql.
-- Do not recreate the policy here: fresh replay already has the policy and PostgreSQL
-- correctly rejects duplicate CREATE POLICY statements.
...
comment on policy product_prices_staff_read on public.product_prices is 'Restricts direct product price reads to authenticated staff; customer catalog pricing remains server-authoritative through get_catalog/create_order.';
