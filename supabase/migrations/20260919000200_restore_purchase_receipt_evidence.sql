-- Restore mandatory durable evidence for purchase receiving after idempotency hardening.
-- This is forward-only: the existing validated receipt semantics remain intact while
...
begin
 ...
 if found then
   select coalesce(jsonb_agg(jsonb_build_object('purchase_order_item_id',x.item_id,'product_id',x.product_id,'quantity',x.quantity) order by x.item_id), '[]'::jsonb) into requested_lines from (select (value->>'purchase_order_item_id')::text item_id,(value->>'product_id')::text product_id,(value->>'quantity')::integer quantity from jsonb_array_elements(p_lines)) x;
   select coalesce(jsonb_agg(jsonb_build_object('purchase_order_item_id',pri.purchase_order_item_id::text,'product_id',pri.product_id::text,'quantity',pri.quantity_received) order by pri.purchase_order_item_id), '[]'::jsonb) into existing_lines from public.purchase_receipt_items pri where pri.organization_id=o and pri.receipt_id=existing.id;
   if existing.purchase_order_id is distinct from p_purchase_order_id or coalesce(existing.notes,'') is distinct from coalesce(nullif(trim(p_notes),''),'') or existing_lines is distinct from requested_lines then raise exception using errcode='40001',message='idempotency key payload conflict'; end if;
 ...
 values(o,auth.uid(),'purchase_receipt.create','purchase_receipt',rec.id,'success',jsonb_build_object('purchase_order_id',po.id,'received_total',total));
 insert into public.outbox_events(organization_id,aggregate_type,aggregate_id,event_type,payload)
 values(o,'purchase_order',po.id,'purchase.received',jsonb_build_object('purchase_order_id',po.id,'receipt_id',rec.id,'receipt_number',rec.receipt_number));
