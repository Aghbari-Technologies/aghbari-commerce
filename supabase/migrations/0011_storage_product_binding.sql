-- Storage hardening: bind every product-media object to a real product in the caller's organization.
-- The existing policy checked only the organization prefix, which allowed a staff actor to
-- create arbitrary object paths and allowed direct reads of guessed paths. Keep the bucket private.
