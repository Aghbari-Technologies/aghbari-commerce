-- Customer invitation lifecycle on the authoritative organization schema.
-- Keep this migration self-contained so a fresh replay never depends on the retired companies model.
...
begin
  ...
  insert into public.customer_invitations(organization_id,customer_id,email,token_hash,expires_at,invited_by)
    values(v_organization,p_customer_id,v_email,encode(digest(v_token,'sha256'),'hex'),v_expires,auth.uid())
    returning id into v_id;
  insert into public.audit_events(organization_id,actor_id,action,target_type,target_id,result,metadata)
    values(v_organization,auth.uid(),'customer_invitation_created','customer_invitation',v_id,'success',
...
begin
  ...
  select c.* into v_customer
    ...
    where c.id=v_inv.customer_id and c.organization_id=v_inv.organization_id
    for update;
  ...
  insert into public.audit_events(organization_id,actor_id,action,target_type,target_id,result,metadata)
    values(v_inv.organization_id,v_uid,'customer_invitation_accepted','customer_invitation',v_inv.id,'success',
