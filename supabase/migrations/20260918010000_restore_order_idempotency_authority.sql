-- Restore the canonical create_order authority after the later runtime-domain migration
-- redefined it with a weaker idempotency path. This migration is intentionally last.
-- Contract: same key + same payload => same order; same key + any payload mismatch => 40001;
-- concurrent same-key requests serialize; stock is locked deterministically; business effects are atomic.
...
BEGIN
  ...
  VALUES(v_org, 'order', v_order.id, 'order.created', pg_catalog.jsonb_build_object('order_id', v_order.id, 'order_number', v_order.order_number));
  INSERT INTO public.audit_events(organization_id, actor_id, action, target_type, target_id, result, metadata)
  VALUES(v_org, auth.uid(), 'order.create', 'order', v_order.id, 'success', pg_catalog.jsonb_build_object('order_number', v_order.order_number, 'cart_converted', v_cart_id IS NOT NULL));
...
COMMENT ON FUNCTION public.create_order(text, uuid, jsonb) IS 'Atomic customer order command with server-authoritative pricing, deterministic stock locking, exact idempotent replay, payload conflict protection, and atomic active-cart conversion.';
