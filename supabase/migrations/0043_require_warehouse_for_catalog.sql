drop function if exists public.get_catalog(text,uuid,integer,integer,uuid);
create function public.get_catalog(p_search text,p_category_id uuid,p_limit integer,p_offset integer,p_warehouse_id uuid)
...
begin
 ...
 return query select p.id,p.sku,p.name,p.unit,p.category_id,p.description,p.status,coalesce((select ib.quantity from public.inventory_balances ib where ib.organization_id=v_org and ib.product_id=p.id and ib.warehouse_id=p_warehouse_id order by ib.updated_at desc limit 1),0),(select pm.storage_path from public.product_media pm where pm.organization_id=v_org and pm.product_id=p.id order by pm.sort_order,pm.created_at limit 1),(select pp.amount from public.product_prices pp join public.price_lists pl on pl.id=pp.price_list_id where pp.organization_id=v_org and pp.product_id=p.id and pl.organization_id=v_org and pl.tier=v_tier and pl.is_active and pp.valid_from<=now() and(pp.valid_to is null or pp.valid_to>now()) order by pp.valid_from desc limit 1),coalesce((select pl.currency from public.price_lists pl where pl.organization_id=v_org and pl.tier=v_tier and pl.is_active limit 1),'YER') from public.products p where p.organization_id=v_org and p.status='active' and(p_category_id is null or p.category_id=p_category_id) and(p_search is null or p_search='' or p.name ilike '%'||p_search||'%' or p.sku ilike '%'||p_search||'%' or coalesce(p.barcode,'')=p_search) order by p.name limit p_limit offset p_offset;
end; $$;
