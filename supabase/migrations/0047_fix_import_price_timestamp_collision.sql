...
begin
 ...
 select * into v_job from public.import_jobs where id=p_import_job_id and organization_id=v_org for update; if not found then raise exception using errcode='P0002',message='import job not found'; end if;
...
grant execute on function public.commit_product_import(uuid,uuid) to authenticated; revoke execute on function public.commit_product_import(uuid,uuid) from anon;
