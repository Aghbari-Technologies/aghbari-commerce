-- Purchase receiving must produce durable operational evidence after its transactional mutation.
...
DECLARE
  o uuid:=public.current_organization_id(); r public.user_role:=public.current_role();
  po public.purchase_orders%rowtype; rec public.purchase_receipts%rowtype; existing public.purchase_receipts%rowtype;
  l jsonb; item public.purchase_order_items%rowtype; pid uuid; q integer; inv integer; total numeric:=0; key text:=trim(coalesce(p_idempotency_key,''));
BEGIN
  ...
  IF FOUND THEN
    RETURN QUERY SELECT existing.id,existing.receipt_number,existing.purchase_order_id,
      (SELECT po0.status FROM public.purchase_orders po0 WHERE po0.id=existing.purchase_order_id),
      coalesce((SELECT sum(pri.line_total) FROM public.purchase_receipt_items pri WHERE pri.organization_id=o AND pri.receipt_id=existing.id),0);
    RETURN;
  END IF;
  ...
  VALUES(o,po.id,po.warehouse_id,key,auth.uid(),nullif(trim(p_notes),'')) RETURNING * INTO rec;
  ...
  SET status=(CASE WHEN NOT EXISTS(SELECT 1 FROM public.purchase_order_items poi WHERE poi.organization_id=o AND poi.purchase_order_id=po.id AND poi.quantity_received<poi.quantity_ordered) THEN 'received'::public.purchase_order_status ELSE 'partially_received'::public.purchase_order_status END),updated_at=now()
  WHERE po0.id=po.id;
  INSERT INTO public.audit_events(organization_id,actor_id,action,target_type,target_id,result,metadata)
  VALUES(o,auth.uid(),'purchase.received','purchase_receipt',rec.id,'success',jsonb_build_object('purchase_order_id',po.id,'receipt_number',rec.receipt_number,'received_total',total));
  INSERT INTO public.outbox_events(organization_id,aggregate_type,aggregate_id,event_type,payload)
  VALUES(o,'purchase_receipt',rec.id,'purchase.received',jsonb_build_object('receipt_id',rec.id,'purchase_order_id',po.id,'received_total',total));
