...
begin
  ...
  for v_row in select * from public.import_rows where import_job_id=v_job.id and organization_id=v_org and status='valid' order by row_number for update loop
    ...
    if found then
      update public.products set name=v_row.normalized_data->>'name',unit=v_row.normalized_data->>'unit',category_id=v_category_id,updated_at=now(),status='active' where id=v_product.id returning * into v_product;
      v_updated := v_updated + 1;
    else
      insert into public.products(organization_id,category_id,sku,name,unit,status) values(v_org,v_category_id,v_row.normalized_data->>'sku',v_row.normalized_data->>'name',v_row.normalized_data->>'unit','active') returning * into v_product;
    ...
    for v_price_list_id in select id from public.price_lists where organization_id=v_org and tier in ('retail','wholesale','distributor') order by tier loop
      update public.product_prices set valid_to=v_effective_at
      where organization_id=v_org and price_list_id=v_price_list_id and product_id=v_product.id and valid_from < v_effective_at and (valid_to is null or valid_to > v_effective_at);
    ...
    select quantity into v_old_qty from public.inventory_balances where organization_id=v_org and warehouse_id=p_warehouse_id and product_id=v_product.id for update;
    v_old_qty := coalesce(v_old_qty,0);
    insert into public.inventory_balances(organization_id,warehouse_id,product_id,quantity) values(v_org,p_warehouse_id,v_product.id,v_new_qty)
    on conflict (warehouse_id,product_id) do update set quantity=excluded.quantity,updated_at=now();
    v_delta := v_new_qty - v_old_qty;
    if v_delta <> 0 then
      insert into public.inventory_movements(organization_id,warehouse_id,product_id,delta,source_type,source_id,actor_id) values(v_org,p_warehouse_id,v_product.id,v_delta,'import',v_job.id,auth.uid());
...
comment on function public.commit_product_import(uuid,uuid) is 'Atomically commits validated imports with exact inventory deltas and non-colliding price version timestamps.';
