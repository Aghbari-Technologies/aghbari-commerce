-- Restore the chunked import lifecycle used by the product import UI and
-- required by the current-SHA behavioral proof. Keep all tenant and role
-- checks server-authoritative and fail closed.
...
BEGIN
  ...
  IF nullif(trim(p_source_fingerprint),'') IS NULL
     OR length(trim(p_source_fingerprint)) <> 64
     OR trim(p_source_fingerprint) !~ '^[0-9a-fA-F]{64}$' THEN
  ...
  WHERE organization_id = v_org
    AND source_fingerprint = lower(trim(p_source_fingerprint))
  FOR UPDATE;

  IF FOUND THEN
    IF v_job.total_rows <> p_total_rows THEN
      RAISE EXCEPTION USING errcode='22023', message='existing import fingerprint has a different row count';
    END IF;
    IF v_job.status IN ('completed','cancelled') THEN
      RAISE EXCEPTION USING errcode='23505', message='duplicate completed import fingerprint';
    END IF;
    RETURN v_job.id;
  END IF;
  ...
  VALUES(
    v_org, trim(p_source_name), lower(trim(p_source_fingerprint)), 'validating', p_total_rows, auth.uid()
  )
...
BEGIN
  ...
  IF p_rows IS NULL OR jsonb_typeof(p_rows) <> 'array'
     OR jsonb_array_length(p_rows) < 1 OR jsonb_array_length(p_rows) > 5000 THEN
  ...
  FOR v_row IN SELECT value FROM jsonb_array_elements(p_rows) LOOP
    ...
    ELSIF length(v_sku) > 80 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','sku','message','SKU is too long'));
    END IF;
    IF v_name = '' THEN
    ...
    ELSIF length(v_name) > 240 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','name','message','Name is too long'));
    END IF;
    IF v_unit = '' THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','unit','message','Unit is required'));
    ELSIF length(v_unit) > 80 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','unit','message','Unit is too long'));
    END IF;
    IF v_category = '' THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','category','message','Category is required'));
    ELSIF length(v_category) > 120 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','category','message','Category is too long'));
    END IF;
    IF v_quantity IS NULL OR v_quantity < 0 OR mod(v_quantity,1) <> 0 OR v_quantity > 10000 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','quantity','message','Quantity is outside the allowed range'));
    END IF;
    IF v_retail IS NULL OR v_retail < 0 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','price.retail','message','Price must be non-negative'));
    END IF;
    IF v_wholesale IS NULL OR v_wholesale < 0 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','price.wholesale','message','Price must be non-negative'));
    END IF;
    IF v_distributor IS NULL OR v_distributor < 0 THEN
      v_diagnostics := v_diagnostics || jsonb_build_array(jsonb_build_object('field','price.distributor','message','Price must be non-negative'));
    END IF;
    IF EXISTS (
      SELECT 1 FROM public.import_rows ir
      WHERE ir.import_job_id = v_job.id
        AND ir.organization_id = v_org
        AND ir.row_number <> v_number
        AND ir.normalized_data->>'sku' = v_sku
        AND v_sku <> ''
    ) THEN
    ...
    v_existing := NULL;
    SELECT raw_data INTO v_existing
    FROM public.import_rows
    WHERE import_job_id = v_job.id
      AND row_number = v_number;

    IF v_existing IS NOT NULL THEN
      IF v_existing <> v_row THEN
        RAISE EXCEPTION USING errcode='40001', message='chunk retry conflicts with an existing row';
      END IF;
    ELSE
      ...
      VALUES(
        v_org, v_job.id, v_number, v_row,
        jsonb_build_object(
          'sku',v_sku,'name',v_name,'unit',v_unit,'category',v_category,
          'quantity',v_quantity,
...
BEGIN
  ...
  END IF;
  IF v_job.status NOT IN ('validating','preview') THEN
    RAISE EXCEPTION USING errcode='P0001', message='import job is not finalizable';
  END IF;
  IF (
    SELECT count(*) FROM public.import_rows
    WHERE import_job_id = v_job.id AND organization_id = v_org
  ) <> v_job.total_rows THEN
  ...
  SET status = 'preview',
      valid_rows = (
        SELECT count(*) FROM public.import_rows
        WHERE import_job_id = v_job.id AND organization_id = v_org AND status = 'valid'
      ),
      invalid_rows = (
        SELECT count(*) FROM public.import_rows
        WHERE import_job_id = v_job.id AND organization_id = v_org AND status = 'invalid'
      ),
      error_summary = coalesce((
        SELECT jsonb_agg(diagnostics) FROM public.import_rows
        WHERE import_job_id = v_job.id AND organization_id = v_org AND status = 'invalid'
      ), '[]'::jsonb)
  ...
  VALUES(
    v_org, auth.uid(), 'import.finalize', 'import_job', v_job.id, 'success',
    jsonb_build_object('rows',v_job.total_rows,'valid',v_job.valid_rows,'invalid',v_job.invalid_rows)
  );
