-- Fix PL/pgSQL output-parameter ambiguity for order_id in idempotent replay lookup.
...
declare
  ...
  v_order public.orders%rowtype; v_existing public.orders%rowtype; v_line jsonb; v_product uuid; v_qty integer;
  v_price numeric(18,2); v_tier public.customer_tier; v_currency text; v_subtotal numeric(18,2) := 0;
  v_available integer; v_line_count integer; v_existing_count integer; v_requested_key text := trim(coalesce(p_idempotency_key,''));
begin
  ...
  if length(v_requested_key)<16 then raise exception using errcode='22023',message='invalid idempotency key'; end if;
  ...
  v_line_count:=jsonb_array_length(p_lines); if v_line_count=0 or v_line_count>100 then raise exception using errcode='22023',message='order must contain between 1 and 100 lines'; end if;
  ...
  for v_line in select value from jsonb_array_elements(p_lines) loop
    if nullif(trim(v_line->>'product_id'),'') is null then raise exception using errcode='22023',message='product_id required'; end if;
    ...
    v_qty:=(v_line->>'quantity')::integer; if v_qty<=0 or v_qty>10000 then raise exception using errcode='22023',message='invalid quantity'; end if;
  end loop;
  if exists(select 1 from(select value->>'product_id' as product_id from jsonb_array_elements(p_lines)) lines group by product_id having count(*)>1) then raise exception using errcode='22023',message='duplicate product line'; end if;
