...
begin
  ...
  if exists (
    select 1 from (
      select (x->>'product_id') as product_id, count(*) as n
      from jsonb_array_elements(p_lines) x group by (x->>'product_id') having count(*) > 1
    ) duplicates
  ) then
  ...
  -- Serialize competing attempts for the same business idempotency key.
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    where organization_id=v_org and warehouse_id=p_warehouse_id and product_id=v_product for update;
  ...
  values(v_org, v_customer, p_warehouse_id, 'pending', v_currency, v_subtotal, v_subtotal, p_idempotency_key, auth.uid())
