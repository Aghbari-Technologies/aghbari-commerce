...
DECLARE
  ...
  v_ui_config jsonb := '{}'::jsonb;
  v_min_order_value numeric := 0;
  v_max_order_value numeric := 0;
BEGIN
  ...
  SELECT coalesce(config, '{}'::jsonb) INTO v_ui_config
  FROM public.client_ui_settings
  WHERE organization_id = v_org;
  IF v_requested_payment = 'credit' AND lower(coalesce(v_ui_config->>'paymentOnCredit','true')) = 'false' THEN RAISE EXCEPTION USING errcode='42501', message='payment method not enabled'; END IF;
  IF v_requested_payment = 'cash' AND lower(coalesce(v_ui_config->>'paymentCash','true')) = 'false' THEN RAISE EXCEPTION USING errcode='42501', message='payment method not enabled'; END IF;
  IF v_requested_payment = 'transfer' AND lower(coalesce(v_ui_config->>'paymentTransfer','true')) = 'false' THEN RAISE EXCEPTION USING errcode='42501', message='payment method not enabled'; END IF;
  ...
  v_min_order_value := CASE WHEN jsonb_typeof(v_ui_config->'minOrderValue') = 'number' THEN (v_ui_config->>'minOrderValue')::numeric ELSE 0 END;
  v_max_order_value := CASE WHEN jsonb_typeof(v_ui_config->'maxOrderValue') = 'number' THEN (v_ui_config->>'maxOrderValue')::numeric ELSE 0 END;
  IF v_min_order_value > 0 AND v_subtotal < v_min_order_value THEN RAISE EXCEPTION USING errcode='P0001', message='minimum order value not met'; END IF;
  IF v_max_order_value > 0 AND v_subtotal > v_max_order_value THEN RAISE EXCEPTION USING errcode='P0001', message='maximum order value exceeded'; END IF;
