-- Fix a real runtime defect: output parameter purchase_order_id shadowed
-- purchase_order_items.purchase_order_id inside receive_purchase_order.
...
BEGIN
 ...
 END LOOP;
 UPDATE public.purchase_orders po0 SET status=CASE WHEN NOT EXISTS(SELECT 1 FROM public.purchase_order_items poi WHERE poi.organization_id=o AND poi.purchase_order_id=po.id AND poi.quantity_received<poi.quantity_ordered) THEN 'received' ELSE 'partially_received' END,updated_at=now() WHERE po0.id=po.id;
