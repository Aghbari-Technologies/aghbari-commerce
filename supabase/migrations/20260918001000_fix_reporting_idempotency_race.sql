...
-- Close the idempotency check/insert race in reporting publication.
-- The unique constraint is the concurrency authority; ON CONFLICT makes a loser
-- wait for the winner and continue with the committed canonical row instead of
-- surfacing a duplicate-key failure to the caller.
...
declare
  ...
  v_source_dataset_id text := trim(p_source_dataset_id);
  v_source_version text := trim(p_source_version);
...
begin
  ...
  if nullif(v_source_dataset_id, '') is null or length(v_source_dataset_id) > 128 then
    raise exception 'Invalid source dataset identity' using errcode = '22023';
  end if;
  if nullif(v_source_version, '') is null or length(v_source_version) > 64 then
  ...
  if nullif(v_idempotency_key, '') is null or length(v_idempotency_key) < 8 or length(v_idempotency_key) > 128 then
