...
revoke all on function public.create_customer_invitation(uuid,text) from public, anon;
revoke all on function public.get_customer_invitation_for_acceptance(text) from public;
revoke all on function public.consume_customer_invitation(text,uuid) from public, anon, authenticated;
