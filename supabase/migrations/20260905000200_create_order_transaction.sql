-- Transactional create-order boundary.
-- Preconditions: migration 20260905000100_core_operational.sql has been applied.
-- This function is intentionally the first real persistence vertical slice.

alter table public.organizations
  add column if not exists next_order_number bigint not null default 1 check (next_order_number > 0);
...
begin
  ...
  if v_customer_status <> 'APPROVED' then
    raise exception 'customer is not approved' using errcode = '42501';
  end if;
  ...
  -- The complete request, including line order, is hashed for deterministic replay diagnostics.
  ...
  -- Serialize only requests that share the same operation key.
  ...
  if found then
    if not exists (
      ...
      where a.organization_id = v_org_id
        and a.target_id = v_existing.id
        and a.action = 'order.create.replay'
        and a.metadata->>'payload_hash' = v_payload_hash
    ) then
  ...
  if v_item_count <> v_seen then
    raise exception 'duplicate product in order lines' using errcode = '23505';
  end if;

  -- Lock inventory rows in deterministic product-id order to reduce deadlock risk.
  for v_product in
  ...
  loop
    ...
    where pp.organization_id = v_org_id
      and pp.product_id = v_product.product_id
      and pl.tier_id = v_tier_id
      and pl.organization_id = v_org_id
      and pp.active and pl.active
    ...
    if v_price is null then
      raise exception 'authorized price not found' using errcode = 'P0002';
    end if;
  ...
  -- Allocate the human-facing order number atomically inside the same transaction.
  ...
  insert into public.orders (
    ...
    order_number, operation_id, status, subtotal, total, version
  ) values (
    v_order_id, v_org_id, p_customer_id, p_branch_id, p_warehouse_id,
    v_order_number, p_operation_id, 'NEW', v_subtotal, v_subtotal, 1
  );
  ...
  loop
    ...
    join public.price_lists pl on pl.id = pp.price_list_id
    where pp.organization_id = v_org_id
      and pp.product_id = v_line.product_id
      and pl.tier_id = v_tier_id
      and pl.organization_id = v_org_id
      and pp.active and pl.active
      and pp.effective_from <= now()
    ...
    insert into public.inventory_movements(
      ...
      quantity_delta, source_type, source_id, operation_id,
      actor_user_id, reason, occurred_at
    ) values (
      v_org_id, p_warehouse_id, v_line.product_id, 'SALE',
      -v_line.quantity, 'ORDER', v_order_id::text, p_operation_id,
      p_actor_user_id, 'Order acceptance', now()
    );
  end loop;
  ...
  insert into public.outbox_events(
    ...
    v_event_id, v_org_id, 'order.created', 1, 'order', v_order_id,
  ...
  return query select v_order_id, v_order_number, 'NEW'::public.order_status,
    v_subtotal, v_subtotal, false;
end;
$$;

comment on function public.create_order_transaction is
  'Atomic canonical order acceptance: scope validation, authorized pricing, inventory lock/decrement, immutable order facts, movement ledger, audit, and outbox.';
