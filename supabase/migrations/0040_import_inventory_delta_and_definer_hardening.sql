-- Correct import inventory movement accounting and harden import SECURITY DEFINER functions.
-- Historical migrations remain immutable; this is an additive correction at the current boundary.
...
begin
  ...
  for v_row in select * from public.import_rows where import_job_id=v_job.id and organization_id=v_org and status='valid' order by row_number for update loop
    ...
    select ib.quantity into v_old_qty
    from public.inventory_balances ib
    where ib.organization_id=v_org and ib.warehouse_id=p_warehouse_id and ib.product_id=v_product.id
    for update;
