...
DECLARE
  ...
  v_existing public.operational_invoices%rowtype;
...
BEGIN
  ...
  VALUES(v_org,auth.uid(),'invoice.create','operational_invoice',v_invoice.id,'success',jsonb_build_object('order_id',v_order.id,'invoice_number',v_invoice.invoice_number));
  INSERT INTO public.outbox_events(organization_id,aggregate_type,aggregate_id,event_type,payload)
  VALUES(v_org,'operational_invoice',v_invoice.id,'invoice.issued',jsonb_build_object('invoice_id',v_invoice.id,'invoice_number',v_invoice.invoice_number,'order_id',v_order.id));
  RETURN v_invoice;
END; $$;
...
BEGIN
  ...
  IF p_method='cash' THEN
    ...
    IF v_account.currency<>v_invoice.currency THEN RAISE EXCEPTION USING errcode='22023',message='cash account currency mismatch'; END IF;
...
BEGIN
  ...
  SELECT * INTO v_account FROM public.cash_accounts WHERE id=p_cash_account_id AND organization_id=v_org AND is_active FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION USING errcode='P0002',message='cash account not found'; END IF;
  IF v_account.currency<>upper(trim(coalesce(p_currency,''))) THEN RAISE EXCEPTION USING errcode='22023',message='cash account currency mismatch'; END IF;
...
LANGUAGE sql STABLE AS $$
  SELECT a.id,a.name,a.currency,a.opening_balance,
    coalesce(sum(CASE WHEN t.direction='in' THEN t.amount ELSE 0 END),0) AS received,
    coalesce(sum(CASE WHEN t.direction='out' THEN t.amount ELSE 0 END),0) AS spent,
    a.opening_balance + coalesce(sum(CASE WHEN t.direction='in' THEN t.amount WHEN t.direction='out' THEN -t.amount ELSE 0 END),0) AS current_balance
  FROM public.cash_accounts a
  LEFT JOIN public.cash_transactions t ON t.organization_id=a.organization_id AND t.cash_account_id=a.id
