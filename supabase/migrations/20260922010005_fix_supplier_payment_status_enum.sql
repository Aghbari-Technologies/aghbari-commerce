...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(v_org::text||':supplier-payment:'||v_key,0));
  ...
  v_hash := pg_catalog.md5(
    v_bill.id::text || '|' || round(p_amount,2)::text || '|' || p_method::text || '|' ||
    coalesce(p_cash_account_id::text,'') || '|' || coalesce(v_reference,'')
  );
  ...
  set status=(
    case when v_paid>=total then 'paid' else 'partially_paid' end
