-- Forward fix for a real clean-source defect: the transfer_inventory RETURNS TABLE
-- output parameter `transfer_id` conflicted with the unqualified table column in the
-- existing-idempotency lookup. Keep the public return column unchanged while qualifying
-- the table column everywhere in the repaired body.
