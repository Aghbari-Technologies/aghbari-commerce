-- Bound mutable staff command inputs at the database boundary.
-- Client-side validation remains UX only; these invariants are enforced server-side.
...
begin
  ...
  if v_name = '' or char_length(v_name) > 120 or v_slug !~ '^[a-z0-9]+(?:-[a-z0-9]+)*$' or char_length(v_slug) > 120 then
  ...
  values(v_org,p_parent_id,v_name,v_slug) returning * into v_category;
...
begin
  ...
  if v_sku = '' or v_name = '' or v_unit = '' or char_length(v_sku) > 80 or char_length(v_name) > 240 or char_length(v_unit) > 80 or char_length(coalesce(v_description,'')) > 2000 then
    raise exception using errcode='22023', message='invalid product input';
  end if;
  ...
  if p_product_id is null then
    ...
    values(v_org,p_category_id,v_sku,v_name,v_unit,v_description,p_status) returning * into v_product;
  else
    update public.products set category_id=p_category_id,sku=v_sku,name=v_name,unit=v_unit,description=v_description,status=p_status,updated_at=now()
...
begin
  ...
  if v_amount is null or v_amount < 0 or p_amount::text in ('NaN','Infinity','-Infinity') or v_currency !~ '^[A-Z]{3}$' then
  ...
  select id, currency into v_price_list, v_list_currency from public.price_lists where organization_id=v_org and tier=p_tier and is_active for update;
  if v_price_list is null then raise exception using errcode='P0002', message='active price list not found'; end if;
  if upper(trim(v_list_currency)) <> v_currency then raise exception using errcode='22023', message='price currency must match the active price list currency'; end if;
  ...
  insert into public.audit_events(organization_id,actor_id,action,target_type,target_id,result,metadata)
  values(v_org,auth.uid(),'product_price.set','product',p_product_id,'success',jsonb_build_object('tier',p_tier,'amount',v_amount,'currency',v_currency));
  return v_amount;
end;
$$;
...
begin
  ...
  if p_delta is null or p_delta = 0 or v_reason = '' or char_length(v_reason) > 500 then raise exception using errcode='22023', message='non-zero delta and reason are required'; end if;
  ...
  select quantity into v_quantity from public.inventory_balances where organization_id=v_org and warehouse_id=p_warehouse_id and product_id=p_product_id for update;
  if v_quantity + p_delta < 0 then raise exception using errcode='P0001', message='inventory cannot become negative'; end if;
  update public.inventory_balances set quantity=v_quantity+p_delta,updated_at=now() where organization_id=v_org and warehouse_id=p_warehouse_id and product_id=p_product_id;
