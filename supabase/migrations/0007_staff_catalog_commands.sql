...
begin
  ...
  if p_parent_id is not null and not exists (
    select 1 from public.categories where id=p_parent_id and organization_id=v_org and is_active
  ) then
  ...
  values(v_org,p_parent_id,trim(p_name),v_slug)
...
begin
  ...
  if p_category_id is not null and not exists (
    select 1 from public.categories where id=p_category_id and organization_id=v_org and is_active
  ) then
    raise exception using errcode='22023', message='invalid category';
  end if;
  ...
  if p_product_id is null then
    ...
    values(v_org,p_category_id,v_sku,v_name,v_unit,nullif(trim(p_description),''),p_status)
    returning * into v_product;
  else
    update public.products
    set category_id=p_category_id, sku=v_sku, name=v_name, unit=v_unit,
        description=nullif(trim(p_description),''), status=p_status, updated_at=now()
...
begin
  if v_org is null or v_role not in ('owner','admin','sales') then
    raise exception using errcode='42501', message='staff pricing access required';
  end if;
  if v_amount is null or v_amount < 0 or p_currency is null or length(trim(p_currency)) <> 3 then
  ...
  select id into v_price_list from public.price_lists
  where organization_id=v_org and tier=p_tier and is_active for update;
  ...
  set valid_to=v_now
  where organization_id=v_org and price_list_id=v_price_list and product_id=p_product_id
...
begin
  if v_org is null or v_role not in ('owner','admin','warehouse') then
    raise exception using errcode='42501', message='warehouse access required';
  end if;
  if p_delta is null or p_delta = 0 or v_reason = '' then
    raise exception using errcode='22023', message='non-zero delta and reason are required';
  end if;
  ...
  values(v_org,p_warehouse_id,p_product_id,0)
  ...
  if v_quantity + p_delta < 0 then
    raise exception using errcode='P0001', message='inventory cannot become negative';
  end if;
  ...
  values(v_org,p_warehouse_id,p_product_id,p_delta,'manual_adjustment',auth.uid(),auth.uid());
  insert into public.audit_events(organization_id,actor_id,action,target_type,target_id,result,metadata)
