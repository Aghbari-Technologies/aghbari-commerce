...
update public.order_templates t
...
create index if not exists idx_order_templates_org_customer_updated
  on public.order_templates(organization_id, customer_id, updated_at desc);
...
create index if not exists idx_order_template_lines_org_template
  on public.order_template_lines(organization_id, template_id);
create index if not exists idx_order_template_lines_product
  on public.order_template_lines(organization_id, product_id);

-- Backfill only rows that have a structurally valid legacy JSON array.
insert into public.order_template_lines(organization_id, template_id, product_id, quantity)
select t.organization_id, t.id, x.product_id, x.quantity
from public.order_templates t
cross join lateral jsonb_to_recordset(
  case when jsonb_typeof(t.lines) = 'array' then t.lines else '[]'::jsonb end
) as x(product_id uuid, quantity integer)
where x.product_id is not null
  and x.quantity between 1 and 1000000
...
create index if not exists idx_order_template_apply_ops_template
  on public.order_template_apply_operations(organization_id, template_id, created_at desc);
...
-- Replace the customer template policies with tenant + customer predicates.
drop policy if exists order_templates_select_own on public.order_templates;
create policy order_templates_select_own on public.order_templates for select to authenticated
using (organization_id = (select organization_id from public.profiles where id = auth.uid()) and customer_id = (select customer_id from public.profiles where id = auth.uid()));
drop policy if exists order_templates_insert_own on public.order_templates;
create policy order_templates_insert_own on public.order_templates for insert to authenticated
with check (organization_id = (select organization_id from public.profiles where id = auth.uid()) and customer_id = (select customer_id from public.profiles where id = auth.uid()));
drop policy if exists order_templates_update_own on public.order_templates;
create policy order_templates_update_own on public.order_templates for update to authenticated
using (organization_id = (select organization_id from public.profiles where id = auth.uid()) and customer_id = (select customer_id from public.profiles where id = auth.uid()))
with check (organization_id = (select organization_id from public.profiles where id = auth.uid()) and customer_id = (select customer_id from public.profiles where id = auth.uid()));
drop policy if exists order_templates_delete_own on public.order_templates;
create policy order_templates_delete_own on public.order_templates for delete to authenticated
using (organization_id = (select organization_id from public.profiles where id = auth.uid()) and customer_id = (select customer_id from public.profiles where id = auth.uid()));
...
begin
  ...
  insert into public.order_templates(organization_id, customer_id, name, branch_label, lines)
  values(v_org, v_customer, trim(p_name), nullif(trim(p_branch_label), ''), '[]'::jsonb)
...
create or replace function public.apply_order_template(
  p_template_id uuid,
  p_warehouse_id uuid,
...
declare
  ...
  v_customer uuid := public.current_customer_id();
  v_cart uuid;
  v_existing public.order_template_apply_operations%rowtype;
  v_hash text;
...
begin
  ...
  if found then
    ...
    return query select v_existing.cart_id, (select count(*)::integer from public.order_template_lines where template_id=p_template_id);
    return;
  end if;
  ...
  if v_cart is null then
    insert into public.carts(organization_id,customer_id,status) values(v_org,v_customer,'active') returning id into v_cart;
  end if;

  -- Full validation happens before any cart mutation. Any failure rolls back the whole function transaction.
  for v_line in select otl.product_id, otl.quantity, p.sku, p.status from public.order_template_lines otl join public.products p on p.id=otl.product_id and p.organization_id=v_org where otl.organization_id=v_org and otl.template_id=p_template_id order by otl.created_at
  loop
    ...
    select pp.amount into v_price from public.product_prices pp join public.price_lists pl on pl.id=pp.price_list_id where pp.organization_id=v_org and pp.product_id=v_line.product_id and pl.organization_id=v_org and pl.tier=v_tier and pl.is_active and pp.valid_from <= now() and (pp.valid_to is null or pp.valid_to > now()) order by pp.valid_from desc limit 1;
    if v_price is null then raise exception using errcode='42501', message=format('no authorized price for SKU %s',v_line.sku); end if;
    v_count := v_count + 1;
  end loop;
  if v_count = 0 then raise exception using errcode='22023', message='template has no valid lines'; end if;

  -- Commit all cart lines atomically after the complete validation pass.
  for v_line in select product_id, quantity from public.order_template_lines where organization_id=v_org and template_id=p_template_id order by created_at
  loop
    insert into public.cart_items(organization_id,cart_id,product_id,quantity)
    values(v_org,v_cart,v_line.product_id,v_line.quantity)
  ...
  insert into public.order_template_apply_operations(organization_id,customer_id,template_id,idempotency_key,request_hash,cart_id)
  values(v_org,v_customer,p_template_id,trim(p_idempotency_key),v_hash,v_cart);
  insert into public.audit_events(organization_id,actor_id,action,target_type,target_id,result,metadata)
  values(v_org,auth.uid(),'order_template.apply','order_template',p_template_id,'success',jsonb_build_object('cart_id',v_cart,'line_count',v_count));
