-- Queue consumer primitives. Authenticated staff may consume only their own tenant;
-- service-role workers may consume all tenants. Claims are atomic and use SKIP LOCKED.
...
BEGIN
  ...
  WITH expired AS (
    ...
    WHERE status='processing'
      ...
      AND (v_is_service OR organization_id=v_org)
    ORDER BY locked_until, created_at, id
    LIMIT v_limit
    FOR UPDATE SKIP LOCKED
  )
  UPDATE public.outbox_events e
...
COMMENT ON FUNCTION public.claim_outbox_events(integer) IS 'Atomically claims tenant-scoped outbox work with a two-minute lease; service-role workers may claim across tenants.';
COMMENT ON FUNCTION public.ack_outbox_event(uuid) IS 'Acknowledges one processing outbox event inside the worker authorization boundary.';
COMMENT ON FUNCTION public.fail_outbox_event(uuid,text) IS 'Returns failed work to pending with bounded exponential backoff or marks it dead at the terminal attempt.';
COMMENT ON FUNCTION public.recover_expired_outbox_events(integer) IS 'Recovers expired worker leases without duplicating already delivered events.';
