...
select ok(public.stage_product_import('tenant-a.xlsx',repeat('a',64),'[{"sku":"IMP-A","name":"Imported A","unit":"unit","category":"Imported","quantity":4,"prices":{"retail":10,"wholesale":9,"distributor":8}}]'::jsonb) is not null,'Tenant A can stage an import job');
...
select ok(public.stage_product_import('tenant-b.xlsx',repeat('a',64),'[{"sku":"IMP-B","name":"Imported B","unit":"unit","category":"Imported","quantity":3,"prices":{"retail":11,"wholesale":10,"distributor":9}}]'::jsonb) is not null,'Tenant B can independently use the same fingerprint');
