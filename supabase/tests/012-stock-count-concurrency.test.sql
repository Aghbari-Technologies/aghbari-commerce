...
select ok((select (public.start_stock_count('24242424-2424-4242-8242-242424242424','count-key-001'))->>'id') is not null,'Starting a stock count returns the created session id');
...
select throws_ok($$select public.set_stock_count_line((select id from public.stock_count_sessions where organization_id='22222222-2222-4222-222222222222' and status='completed'),'25252525-2525-4252-8252-252525252525',13)$$,'22023','stock count is not open','Completed stock counts cannot be modified');
select * from finish();
rollback;
