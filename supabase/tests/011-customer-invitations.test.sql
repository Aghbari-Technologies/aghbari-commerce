...
select ok((select p.proconfig @> array['search_path=public']::text[] from pg_proc p where p.oid='public.create_customer_invitation(uuid,text)'::regprocedure), 'create invitation pins search_path');
select ok((select p.proconfig @> array['search_path=public']::text[] from pg_proc p where p.oid='public.consume_customer_invitation(text,uuid)'::regprocedure), 'consume invitation pins search_path');
