...
select throws_ok($$select public.start_stock_count('67676767-6767-4676-8676-676767676771','stock-count-idem-01')$$,'55006',null,'An idempotency key cannot open a second active count for the same tenant');
...
select is((public.complete_stock_count((select id from public.stock_count_sessions where idempotency_key='stock-count-idem-01'))).status,'completed'::public.stock_count_status,'Completed count closes the session');
