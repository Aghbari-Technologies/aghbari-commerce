...
select throws_ok($$select public.start_stock_count('67676767-6767-4676-8676-676767676769','stock-count-idem-02')$$,'55006','an open stock count already exists for this warehouse','A second open stock count for the same warehouse is rejected');
...
select is((select (public.start_stock_count('67676767-6767-4676-8676-676767676769','stock-count-idem-01')).status),'completed'::public.stock_count_status,'Replaying the same start key is idempotent and returns the completed session');
