...
insert into public.inventory_balances(organization_id,warehouse_id,product_id,quantity) values ('67676767-6767-4676-8676-676767676767','67676767-6767-4676-8676-676767676769','67676767-6767-4676-8676-676767676770',10);
...
select is((select status from public.start_stock_count('67676767-6767-4676-8676-676767676769','stock-count-idem-01','cycle count test')),'open','Starting a stock count creates an open session');
...
select throws_ok($$select public.complete_stock_count((select id from public.stock_count_sessions where idempotency_key='stock-count-idem-01'))$$,'22023',null,'Completion is blocked until every line is counted');
select throws_ok($$select public.start_stock_count('67676767-6767-4676-8676-676767676771','stock-count-idem-01')$$,'23505',null,'An idempotency key cannot be replayed against another warehouse');
...
select is((public.complete_stock_count((select id from public.stock_count_sessions where idempotency_key='stock-count-idem-01'))->>'status'),'completed','Completed count closes the session');
...
select is((select status from public.start_stock_count('67676767-6767-4676-8676-676767676769','stock-count-idem-01')),'completed','Replaying the same start key is idempotent and returns the completed session');
