...
select is((select count(*) from public.outbox_events where event_type='purchase.received' and aggregate_id=(select id from public.purchase_receipts where idempotency_key='r5-receipt-key-000001')),1::bigint,'Receiving emits one durable outbox event tied to the exact receipt aggregate');
set local role authenticated;
