...
insert into public.warehouses (id, organization_id, branch_id, name) values ('eeeeeeee-eeee-4eee-8eee-eeeeeeeeee02', 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee', 'Main Warehouse');
...
select is((select count(*) from public.outbox_events where event_type='purchase.received' and payload->>'receipt_id'=(select id::text from public.purchase_receipts where idempotency_key='r5-receipt-key-000001')),1::bigint,'Receiving emits one durable outbox event tied to the exact receipt payload');
set local role authenticated;
