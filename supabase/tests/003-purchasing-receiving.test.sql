...
insert into public.warehouses (id, organization_id, branch_id, name) values ('eeeeeeee-eeee-4eee-8eee-eeeeeeeeee02', 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee', 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeee01', 'Main Branch Warehouse');
...
select throws_ok($$select * from public.create_purchase_order('eeeeeeee-eeee-4eee-8eee-eeeeeeeeee05'::uuid,'eeeeeeee-eeee-4eee-8eee-eeeeeeeeee02'::uuid,'r5-purchase-key-000001',jsonb_build_array(jsonb_build_object('product_id','eeeeeeee-eeee-4eee-8eee-eeeeeeeeee03','quantity',11,'unit_cost',1250)),'YER')$$,'40001',null,'Changed purchase payload under the same key is rejected');
...
select is((select purchase_order_status from public.receive_purchase_order((select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001'),'r5-receipt-key-000001',jsonb_build_array(jsonb_build_object('purchase_order_item_id',(select id from public.purchase_order_items where purchase_order_id=(select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001') limit 1),'product_id','eeeeeeee-eeee-4eee-8eee-eeeeeeeeee03','quantity',10)))), 'received'::public.purchase_order_status,'Full receipt closes the purchase order');
...
select is((select count(*) from public.outbox_events where event_type='purchase.received' and aggregate_type='purchase_receipt' and aggregate_id=(select id from public.purchase_receipts where idempotency_key='r5-receipt-key-000001')),1::bigint,'Receiving emits one durable outbox event tied to the exact receipt aggregate');
set local role authenticated;
select throws_ok($$select * from public.receive_purchase_order((select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001'),'r5-receipt-key-000001',jsonb_build_array(jsonb_build_object('purchase_order_item_id',(select id from public.purchase_order_items where purchase_order_id=(select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001') limit 1),'product_id','eeeeeeee-eeee-4eee-8eee-eeeeeeeeee03','quantity',9)))$$,'40001',null,'Changed receipt payload under the same key is rejected');
select results_eq($$select received_total from public.receive_purchase_order((select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001'),'r5-receipt-key-000001',jsonb_build_array(jsonb_build_object('purchase_order_item_id',(select id from public.purchase_order_items where purchase_order_id=(select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001') limit 1),'product_id','eeeeeeee-eeee-4eee-8eee-eeeeeeeeee03','quantity',10)))$$,$$values (12500::numeric)$$,'Receipt replay is idempotent');
