...
select set_config('request.jwt.claims',json_build_object('role','authenticated','sub','44444444-4444-4444-8444-444444444444')::text,true);
...
select throws_ok($$select * from public.receive_purchase_order((select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001'),'r5-receipt-key-000001',jsonb_build_array(jsonb_build_object('purchase_order_item_id',(select id from public.purchase_order_items limit 1),'product_id','eeeeeeee-eeee-4eee-8eee-eeeeeeeeee03','quantity',9)))$$,'40001',null,'Changed receipt payload under the same key is rejected');
