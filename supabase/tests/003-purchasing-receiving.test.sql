...
select results_eq(
  ...
  $$values (1::bigint)$$,
  'Purchase order is created with server-generated number'
);

select results_eq(
  $$select quantity_ordered from public.purchase_order_items where purchase_order_id=(select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001')$$,
  $$values (10::integer)$$,
  'Purchase order line is persisted'
);
...
select results_eq(
  ...
  $$values (12500::numeric)$$,
  'Exact purchase replay returns the original total'
);
...
select throws_ok(
  ...
  '40001', 'idempotency key payload conflict', 'Changed purchase payload under the same key is rejected'
);
...
select is((select purchase_order_status from public.receive_purchase_order(
  (select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001'),'r5-receipt-key-000001',
...
select throws_ok(
  ...
  '40001', 'idempotency key payload conflict', 'Changed receipt payload under the same key is rejected'
);
...
select results_eq(
  ...
  $$values (12500::numeric)$$,
  'Receipt replay is idempotent and does not double-mutate inventory'
);
