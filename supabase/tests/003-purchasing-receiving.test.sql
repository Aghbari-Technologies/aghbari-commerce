...
select is((select count(*) from public.outbox_events where event_type='purchase.received' and aggregate_type='purchase_receipt'),1::bigint,'Receiving emits one durable outbox event');
set local role authenticated;
