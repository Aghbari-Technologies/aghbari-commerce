...
select is((select count(*) from public.outbox_events where aggregate_id=(select id from public.purchase_receipts where idempotency_key='r5-receipt-key-000001') and event_type='purchase.received'),1::bigint,'Receiving emits one durable outbox event for the exact receipt');
set local role authenticated;
select throws_ok($$select * from public.receive_purchase_order((select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001'),'r5-receipt-key-000001',jsonb_build_array(jsonb_build_object('purchase_order_item_id',(select id from public.purchase_order_items where purchase_order_id=(select id from public.purchase_orders where idempotency_key='r5-purchase-key-000001') limit 1),'product_id','eeeeeeee-eeee-4eee-8eee-eeeeeeeeee03','quantity',9)))$$,'40001','idempotency key payload conflict','Changed receipt payload under the same key is rejected');
