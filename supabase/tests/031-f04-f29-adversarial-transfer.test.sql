...
insert into public.warehouses (id,organization_id,branch_id,name) values
 ...
 ('22222222-2222-4222-8222-222222222224','22222222-2222-4222-8222-222222222222','22222222-2222-4222-8222-222222222223','B Warehouse');
...
select throws_ok($$select * from public.transfer_inventory('11111111-1111-4111-8111-111111111114','11111111-1111-4111-8111-111111111115','adv-004',jsonb_build_array(jsonb_build_object('product_id','11111111-1111-4111-8111-111111111116','quantity',0)),'zero')$$,'22023','invalid transfer quantity','zero quantity is rejected');
select throws_ok($$select * from public.transfer_inventory('11111111-1111-4111-8111-111111111114','11111111-1111-4111-8111-111111111115','adv-005',jsonb_build_array(jsonb_build_object('product_id','22222222-2222-4222-8222-222222222225','quantity',1)),'cross tenant product')$$,'P0002','product not found','cross-tenant product is rejected');
