...
select lives_ok($$select public.notifications.id from public.notifications limit 1$$,'Customer can query notifications within its tenant');
select is((select count(*) from public.notifications),0::bigint,'Customer starts with no notifications');
select throws_ok($$insert into public.notifications(organization_id,customer_id,kind,title,body) values ('43434343-4343-4343-8343-434343434343','45454545-4545-4545-8545-454545454545','test','Direct write','Must be blocked')$$,'42501',null,'Authenticated customer cannot write notifications directly');

insert into public.notifications(organization_id,customer_id,kind,title,body)
values ('43434343-4343-4343-8343-434343434343','45454545-4545-4545-8545-454545454545','test','Allowed fixture','Tenant A notification');

select is((select count(*) from public.notifications),1::bigint,'Tenant A can read its notification');
