...
insert into public.notifications(organization_id,customer_id,recipient_user_id,kind,title,body)
values ('43434343-4343-4343-8343-434343434343','45454545-4545-4545-8545-454545454545','41414141-4141-4141-8141-414141414141','test','Fixture','Tenant A');
...
select is((select count(*) from public.notifications),1::bigint,'Tenant A sees its own notification');
select ok(exists(select 1 from public.notifications where recipient_user_id=auth.uid()),'Recipient-scoped notification remains visible to its user');
...
select ok(not exists(
  ...
  where organization_id='44444444-4444-4444-8444-444444444444'
),'Tenant A cannot read Tenant B notifications');
