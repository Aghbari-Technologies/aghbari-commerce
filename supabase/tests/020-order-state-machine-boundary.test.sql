...
insert into public.organizations(id,name,is_active) select org_a,'State Tenant A',true from fixture union all select org_b,'State Tenant B',true from fixture;
insert into public.customers(id,organization_id,name,tier,is_active) select customer_a,org_a,'State Customer A','retail'::customer_tier,true from fixture union all select customer_b,org_b,'State Customer B','retail'::customer_tier,true from fixture;
insert into public.profiles(id,organization_id,customer_id,role) select user_a,org_a,customer_a,'viewer'::user_role from fixture union all select user_b,org_b,customer_b,'viewer'::user_role from fixture;
insert into public.branches(id,organization_id,name,is_active) select branch_a,org_a,'State Branch A',true from fixture union all select branch_b,org_b,'State Branch B',true from fixture;
insert into public.warehouses(id,organization_id,branch_id,name,is_active) select warehouse_a,org_a,branch_a,'State Warehouse A',true from fixture union all select warehouse_b,org_b,branch_b,'State Warehouse B',true from fixture;
insert into public.products(id,organization_id,sku,name,unit,status) select product_a,org_a,'STATE-A','State Product A','unit','active' from fixture union all select product_b,org_b,'STATE-B','State Product B','unit','active' from fixture;
insert into public.inventory_balances(organization_id,warehouse_id,product_id,quantity) select org_a,warehouse_a,product_a,10 from fixture union all select org_b,warehouse_b,product_b,10 from fixture;
insert into public.price_lists(organization_id,tier,name,currency) select org_a,'retail'::customer_tier,'State Retail A','YER' from fixture union all select org_b,'retail'::customer_tier,'State Retail B','YER' from fixture;
insert into public.product_prices(organization_id,price_list_id,product_id,amount,valid_from) select org_a,(select id from public.price_lists where organization_id=org_a),product_a,100,now() from fixture union all select org_b,(select id from public.price_lists where organization_id=org_b),product_b,200,now() from fixture;
