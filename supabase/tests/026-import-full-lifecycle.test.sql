...
select throws_ok($$select public.stage_product_import('foreign.xlsx',repeat('e',64),jsonb_build_array(jsonb_build_object('sku','FOREIGN-001','name','Foreign Attempt','unit','unit','category','Foreign','quantity',1,'prices',jsonb_build_object('retail',10,'wholesale',9,'distributor',8))))$$,'42501',null,'foreign tenant cannot stage into Tenant A');
select is((select count(*) from public.import_jobs where organization_id='aaaaaaaa-1111-4111-8111-aaaaaaaa0100' and source_fingerprint=repeat('e',64)),0::bigint,'foreign stage creates no Tenant A job');
