...
insert into public.products(id,organization_id,sku,name,unit,status) values
 ('aaaaaaaa-1111-4111-8111-aaaaaaaa0130','aaaaaaaa-1111-4111-8111-aaaaaaaa0100','PRE-001','Preexisting Product','unit','active');
...
-- Valid two-row lifecycle: begin -> chunk -> chunk retry -> finalize -> commit -> successful completion.
...
select throws_ok($$select public.begin_product_import('valid.xlsx',repeat('a',64),3)$$,'22023',null,'same fingerprint with changed declared row count is rejected');
...
select is((select status from public.import_jobs where source_fingerprint=repeat('a',64)),'validating','failed foreign-target commit does not advance import state');
select lives_ok($$select public.finalize_product_import((select id from public.import_jobs where source_fingerprint=repeat('a',64)))$$,'complete two-row import can be finalized');
select is((select status from public.import_jobs where source_fingerprint=repeat('a',64)),'preview','finalize produces preview state');
select is((select invalid_rows from public.import_jobs where source_fingerprint=repeat('a',64)),0,'valid import has zero invalid rows');
...
-- Invalid schema/input and invalid row are fail-closed and cannot be committed.
...
-- Partial failure/recovery and foreign-tenant actor denial.
