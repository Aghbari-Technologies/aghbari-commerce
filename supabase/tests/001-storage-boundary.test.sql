...
select throws_ok($$select public.register_product_media('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa11'::uuid,'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbb11/bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbb21.webp','image/webp',1200,900,1024)$$,'42501',null,'Server rejects a foreign-tenant storage path');
...
select results_eq($$select array_agg(cmd order by cmd) from pg_policies where schemaname='storage' and tablename='objects' and policyname in ('product_media_select','product_media_insert','product_media_delete')$$,$$values (array['a','d','r']::text[])$$,'Product media lifecycle excludes UPDATE and retains INSERT/DELETE/SELECT boundaries');
