...
select is(has_function_privilege('public', 'public.adjust_inventory(uuid,uuid,integer,text)', 'execute'), false, 'PUBLIC cannot execute adjust_inventory');
...
select is(to_regprocedure('public.get_catalog(text,uuid,integer,integer)'), null, 'legacy 4-arg get_catalog is absent');
select is(to_regprocedure('public.get_catalog(text,uuid,integer,integer)'), null, 'legacy 4-arg get_catalog is absent for anonymous callers');
select is(to_regprocedure('public.get_catalog(text,uuid,integer,integer)'), null, 'legacy 4-arg get_catalog is absent from public surface');
...
select is(has_function_privilege('anon', 'public.transition_order(uuid,public.order_status)', 'execute'), false, 'anon cannot execute transition_order');
select is(has_function_privilege('authenticated', 'public.transition_order(uuid,public.order_status)', 'execute'), true, 'authenticated can execute transition_order');
select is(has_function_privilege('public', 'public.transition_order(uuid,public.order_status)', 'execute'), false, 'PUBLIC cannot execute transition_order');
...
select is(has_function_privilege('anon', 'public.set_product_price(uuid,public.customer_tier,numeric,text)', 'execute'), false, 'anon cannot execute set_product_price');
select is(has_function_privilege('authenticated', 'public.set_product_price(uuid,public.customer_tier,numeric,text)', 'execute'), true, 'authenticated can execute set_product_price');
select is(has_function_privilege('public', 'public.set_product_price(uuid,public.customer_tier,numeric,text)', 'execute'), false, 'PUBLIC cannot execute set_product_price');
