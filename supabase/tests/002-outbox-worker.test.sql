...
select results_eq($$select organization_id from public.claim_outbox_events(10)$$,$$values ('cccccccc-cccc-4ccc-8ccc-cccccccccccc'::uuid)$$,'Tenant-scoped worker claim returns only local events');
select results_eq($$select status from public.outbox_events where organization_id='cccccccc-cccc-4ccc-8ccc-cccccccccccc'$$,$$values ('processing'::text)$$,'Claim transitions the event into processing');
select results_eq($$select attempts from public.outbox_events where organization_id='cccccccc-cccc-4ccc-8ccc-cccccccccccc'$$,$$values (1::integer)$$,'Claim increments attempts exactly once');
select is(public.ack_outbox_event((select id from public.outbox_events where aggregate_id='cccccccc-cccc-4ccc-8ccc-cccccccccc11'::uuid)),true,'Acknowledgement succeeds for the claimed tenant event');
select results_eq($$select status from public.outbox_events where aggregate_id='cccccccc-cccc-4ccc-8ccc-cccccccccc11'::uuid$$,$$values ('delivered'::text)$$,'Acknowledged event becomes delivered');
...
select results_eq($$select status from public.outbox_events where aggregate_id='cccccccc-cccc-4ccc-8ccc-cccccccccc12'::uuid$$,$$values ('pending'::text)$$,'Recovered work returns to pending for retry');
