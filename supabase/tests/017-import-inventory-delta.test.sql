...
select is((select delta from public.inventory_movements where organization_id=(select org_id from fixture) order by created_at desc limit 1),4,'First import records exact delta of four');
...
select is((select delta from public.inventory_movements where organization_id=(select org_id from fixture) order by created_at desc limit 1),3,'Second import records exact delta of three');
