...
select is((select delta from public.inventory_movements where source_id=(select id from public.import_jobs where source_name='one.xlsx' and source_fingerprint=repeat('d',64)) and organization_id=(select org_id from fixture)),4,'First import records exact delta of four');
...
select is((select delta from public.inventory_movements where source_id=(select id from public.import_jobs where source_name='two.xlsx' and source_fingerprint=repeat('e',64)) and organization_id=(select org_id from fixture)),3,'Second import records exact delta of three');
