...
insert into auth.users(id,instance_id,aud,role,email,encrypted_password,created_at,updated_at) select admin_id,'00000000-0000-0000-0000-000000000000'::uuid,'authenticated','authenticated','import-delta@fixture.invalid','x',now(),now() from fixture;
...
select is((select delta from public.inventory_movements where organization_id=(select org_id from fixture) and source_type='import' and source_id=(select id from public.import_jobs where organization_id=(select org_id from fixture) and source_fingerprint='aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa1') limit 1),4,'First import records exact delta of four');
select is((select (public.commit_product_import(public.stage_product_import('two.xlsx','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa2','[{"sku":"DELTA-TEST","name":"Delta Test","unit":"unit","category":"Delta","quantity":7,"prices":{"retail":10,"wholesale":9,"distributor":8}}]'::jsonb),(select warehouse_id from fixture))).inventory_changed),1,'Second import records one inventory change');
...
select is((select delta from public.inventory_movements where organization_id=(select org_id from fixture) and source_type='import' and source_id=(select id from public.import_jobs where organization_id=(select org_id from fixture) and source_fingerprint='aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa2') limit 1),3,'Second import records exact delta of three');
select * from finish();
rollback;
