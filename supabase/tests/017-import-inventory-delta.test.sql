...
select is((select delta from public.inventory_movements where organization_id=(select org_id from fixture) and source_id=(select id from public.import_jobs where organization_id=(select org_id from fixture) and source_fingerprint=repeat('e',64)),3,'Second import records exact delta of three');
