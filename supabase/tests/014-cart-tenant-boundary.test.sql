...
union all select user_b,'00000000-0000-0000-0000-000000000000'::uuid,'authenticated','authenticated','cart-b@fixture.invalid','x',now(),now() from fixture;
insert into public.organizations(id,name,is_active) select org_a,'Cart Tenant A',true from fixture union all select org_b,'Cart Tenant B',true from fixture;
insert into public.customers(id,organization_id,name,tier,is_active) select customer_a,org_a,'Customer A','wholesale'::customer_tier,true from fixture union all select customer_b,org_b,'Customer B','wholesale'::customer_tier,true from fixture;
...
insert into public.products(id,organization_id,sku,name,unit,status) select product_a,org_a,'CART-A','Cart Product A','unit','active' from fixture union all select product_b,org_b,'CART-B','Cart Product B','unit','active' from fixture;
...
select throws_ok(format('select public.set_cart_item(%L,2)',product_b),'P0001',null,'Tenant A cannot add Tenant B product') from fixture;
